...
| G22 | `heatAbsentPolicy` 三种取值、`relaySource` × `relayScoring` 四种组合**各有一条夹具**；代码里不存在「哪个是默认」的分支（**枚举兜底与数字兜底都算**）；示例配置里三个键都是 `"__TO_BE_CALIBRATED__"` | `test_v250_s5_params_guard.py::test_no_default_branch_for_the_three_calibrated_value_slots` + `::test_no_numeric_default_anywhere_in_the_strategy_layer`；夹具矩阵在 `test_k9_params.py` / `test_k9_layer.py` |
