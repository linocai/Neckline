...
- **用户真实数据已在生产**:`positions` **3 笔 open**(2026-07-27 补录,⚠ 买入日被写成当天,§七 P0-1)、`watchlist` **16 只**、`devices` 1 台(macOS)。**再动章程必须走 `scripts/activate_charter.py` 四道闸**;持仓非空时闸 2 只对「纯入场侧 diff」开窄豁免(见本节 07-27 v1.3.3 条),豁免必留痕。
...
- **⚠ 冷启动先核两件事**(本会话未活体确认):① `GET /health` 的版号是不是 `v1.3.5` —— §九 v1.3.5 条目只记了**生产数据修复 + 补跑**,**没有代码 rsync / 重启 / health 的部署留痕**,存在「数据已修、代码未上云」的可能;② 今晚 16:35 报告是否 `Result=success`(**timer 跑过 ≠ 任务成功**,这是 07-27 那次报告没生成却被记成「已跑完」的教训)。
...
- **[P3-11] 打法 × 论点双维归因(原 v1.2.1-D)**。施工图全文在 §五 v1.2.1-D(**保留勿删**):决策日志 via `position_id` 与 FIFO RoundTrip 按 `(ts_code, buy_date)` 邻近匹配,按论点 / 打法 / 交叉三维聚合胜率盈亏比 + 呼吸 T 净贡献 + 「无决策日志开仓 N 笔」纪律项。**为什么要紧**:「短线追击 vs 呼吸试验」双打法 A/B 裁决的**唯一归因载体**;需 30–50 笔样本才有意义。⚠ 依赖 P0-1 修好(买入日错 → 归因的持有周期全错)。**归 v1.4+。**
