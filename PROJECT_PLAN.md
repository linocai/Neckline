...
- **⚠ 待用户拍板**:①**章程激活仍挂起**(用户本次明确不激活)——v1.3 行激活仍待「用户清仓 + 明确确认」后跑 `sudo -u neckline .venv/bin/python scripts/activate_charter.py --target v1.3 --confirm`;在此之前 🔴-1 定格逻辑虽已上线但**恒不触发**(K1 单档)。②v1.3-⑦ 的真机验收清单不因本次快修销账。
