...
**2026-07-27 · 独立审计缺陷修复完工(@builder-pro,🔴 高危区:纪律章程 + 金额判定;**已随 v1.3.1 部署上云,见上条**;未激活任何章程行)。** 依据 `archive/REVIEW_REPORT_v1.3_章程金额审计_20260727.md`,把 v1.3 章程激活前的拦路石全部拆除。
...
- **[✅ v1.3-⑤ 已完工(原 v1.2.1-D2 归 v1.3,2026-07-26)] `api/inquiry.py::run_deterministic_checks` 选股域漂移**:手写重复 `research/panel.py::base_universe_expr()` 选股域逻辑(4A 遗留,v1.1-C/D 施工时发现,原只记 §四正文未进 Backlog)→ 已改为复用 `report/watchlist_check.py::discipline_checks`(原 `_discipline_checks` 提升为公开函数,两处共用同一份;选股域揉一条不拆解,只 config 可配禁买过滤逐项拆)。**顺带修复的真实缺口**:清理前 P4(`forbid_green_bigdown`)/P5(`forbid_far_from_high`)在问询台完全未实现,即便现役 config 打开也不会拦下,现已同码生效。详见 §四/§九 完工条目。
