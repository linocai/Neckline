...
- **③-C3 候选池四步管线改版(需求 5,K1 entry mask 退役)**:新 `report/intel_candidates.py`(或改造 `candidates.py`——但**候选评分 `_base_score_expr` 保留供自选体检/纪律红绿灯复用,不删**):
  ...
  - **④ 情报排序** = 板块资金流强度(C2)+ 题材持续天数**反用**(1 天新鲜 > 2-3 天警惕 > ≥4 天已在 ③ hard_cut 剔)+ 温和带标注 → 出 **20 只**交用户终选。
    ...
    - **⚠ 行业闸:概念当题材 + 行业当闸(用户 2026-07-26 拍板方案二,写死)**:保底/竞争会把「名义上挂在该板块、但与主题无关」的票推上榜(实测:机器人概念栏给出九州通/重药控股〔医药商业〕、稀土永磁栏给出中炬高新〔厨邦酱油·食品〕;芯片概念栏给出瑞晨环保〔机械基件〕/立中集团〔汽车配件〕)——成分归属没错(同花顺沾边挂靠,立中挂 30 板块/九州通挂 25),错在**板块内排序用的全是与主题无关的指标**(资金流/趋势/题材天数),当日最强题材的票就浮到不相干板块前排,**看着像情报实为噪音、比一只不给更糟**。修法:用 `stock_basic.industry`(**一票一行业、无沾边**)对每个板块**自动**算「主导行业集合」= 成员里**行业占比 ≥ `INDUSTRY_GATE_MIN_SHARE`(=0.05,启发式待实盘校准)**的行业(分母=全体成员);个股必须**行业 ∈ 该板块主导行业集合**才能作为该板块**代表票**(**保底与竞争的板块归属都过这道闸**)。**数据驱动、不手配白名单**——对当日暴起板块**自动同样生效**(关键:白名单覆盖不了动态发现的板块)。**无 industry 的票视为不通过闸**(保守,无法判主题相关性),落审计日志、不静默丢。**出参带 `intelRank.industry`**(让客户端说清「这只票凭什么在这个板块栏」)。闸**只作用于板块归属/代表性**,不改卫生线、不改 K4 hard_cut/avoid_flag、不改情报排序公式本身。真实 2026-07-22 验:机器人主导含专用机械 11.4%…医药商业仅 0.3% 被挡、稀土主导含矿物制品 18.5%/小金属 16.9%…食品仅 1.5% 被挡、芯片主导含半导体 21.1%…机械基件/汽车配件 <3% 被挡——五个噪音反例全挡出;某常驻当日无「过闸 + 过 K4」合格票时诚实给 0、缺额退公共池(宁缺毋滥)。落地 `report/intel_candidates.py::_dominant_industries`。
