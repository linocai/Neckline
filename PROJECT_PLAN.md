...
- **⚠ 需用户 + 策略线拍板:K4 行缺席**。生产 `strategy_versions` 只有 K1(K4 从未上生产、无 committed 落库脚本、属策略线 K 版本)→ v1.3-②/③ 的 K4 **hard_cut 分区全降级为 avoid_flag**(打标不拦、机器不禁、**不崩**:阈值住模块命名常量镜像,只 evidence/分区读 DB,缺行走 `_FALLBACK_EVIDENCE`+`_DEFAULT_SECTION=avoid_flag`;持仓强/普通分级仍走模块常量正常)。要让 hard_cut 真正生效,需策略线把 K4 行 bootstrap 到生产权威库(建议附一个可复现落库脚本 + 用户批准)。
