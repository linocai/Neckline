...
**2026-07-29 · v1.4-⑩(P0-23 修复)本地部分完工,生产侧待 17:00 后 —— 生产仍是 `v1.4.0-p1`,现役章程仍 `v1.3.3`(未动)**。①~⑧ + ⑩ 本地代码全部完工:Python 全量 **1677 过 + 2 skip**(基线 1647+2),Swift iOS Simulator **168 跑 155 过 13 skip**(基线 165),双端 `xcodebuild build` SUCCEEDED。⑨ 完整部署 07-29 上午尝试后**已按预案回滚**,阻塞 = §七 **P0-23**,修法(方案②预计算落表)本地已落地并用真数据演练通过。

- **⑩ 本地完工态(细节见 §九 一行)**:新表 `industry_strength_daily` + `report/industry_strength_store.py`(读写单一通道)+ CLI `scripts/industry_strength.py {refresh,verify,bootstrap}` + 16:05 日更挂载;四处在线取数点 + ④分歧线全部**只读表**,守门单测锁死(四文件 grep 零现算入口 + 三路等价)。**本地真数据演练**(scratchpad DB 副本 + 真实 parquet,零生产访问):全历史 bootstrap **3s / 174,790 行 / 1589 交易日 / 110 行业**,库 3.8→**29.0MB**,`verify` 三项全绿,随机 3 日单日对拍逐位一致,`build_info_card` **0.56s**(此前生产永不返回)。演练顺带打出并修掉一个真洞:`rank(ordinal)` 并列由行序打散 → bootstrap 与日更算出不同 rank(已加确定性 tie-break + 纪律 + 单测)。
- **🔴 ⑩ 剩余未做 = 生产侧三步(17:00 后按探针纪律做)**:① 旁路目录 `/opt/neckline-p023/` bootstrap 回填 + ⑩-D 五条判据取证;② ⑩-G 五条实测证据(日更增量 / 读侧代价 / 信息卡主体成本 / `integrity_check` + `.backup` 耗时);③ 跑完删旁路目录 + 留痕 §九 与 `~/hz_info.md`。**未做之前 ⑩ 不算完工,不得进 review 门禁,更不得开 ⑨。**
...
- **工序链(照此走,别跳步)**:⑩ 本地施工 ✅ → **⑩ 生产旁路 bootstrap + 隔离实测(17:00 后)** → ⑩-G 判据全绿 → 用户召唤 @reviewer(上云前审)→ review 结论处置完 → ⑨ 重跑上云 → 双端换包。
- **⑨ 重跑的增量清单**(07-29 那次尝试之上的更新版,原清单其余项已做掉或作废):① 本地 `VERSION` 已是 `v1.4.0`(`api/app.py:163`,回滚时未回改),⑨ 时把该文件里「本版号尚未上云」那段模块注释一并删掉;② `ths_daily` 已补到 0728,**不用再补**,只需重新核实当日进度;③ 迁移清单 = `decision_log.max_chase_pct` + `inquiry_log`(07-29 已落生产、刻意保留)+ **⑩ 新增 `industry_strength_daily`**(若 ⑩ 生产旁路已建表回填,⑨-B 只验存在性 / 行数 / `verify` 三项);④ `MemoryMax` **按 ⑩-G 实测定**(⑨-F 原文「视情抬到 1G」已作废 —— 抬内存不是解);⑤ TuShare 配额账复核照旧(⑩ 新增消耗 = **0**,只读已落盘 parquet);⑥ **iOS/macOS 双端换包**(§八 第 12 项:现在装 v1.4 客户端会可见地坏,必须等服务端真上到 v1.4.0)。
...
- **刻意没做(各有理由,别当遗漏)**:`MemoryMax` 未改(新证据表明抬内存不是解,且 v1.4 已回滚无消费方);macOS/iOS 未换包(v1.4 客户端配 p1 服务端会可见地坏:信息卡 404 / 问询历史 404 / `maxChasePct` 静默丢弃);⑩ 的生产侧动作一律押后到收盘(交易时段零生产访问,守新立的探针纪律)。
...
- **待办总入口 = §七 Backlog**(P0-23 已标「本地修复完工,生产侧待做」)。策略假设 / K 字头版本权威在 `STRATEGY_LAB.md`。①~⑧ + ⑩ 各块完工细节全文见 §九 一行 + `archive/`,本节不复述。
