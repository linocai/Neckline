...
>
> **⚠ K4 持仓牌强警示推送(v1.3 需求 7,planner 默认口径,标注"用户可否决")**:持仓牌每日重算的**强警示级别**(年线下涨停 / 放量大阳 = 派发警报、题材持续 ≥4 天、换手 >10%)推 APNs;**普通警示**(题材 2-3 天等)只进盘中看板。**不新增第六类推送**——K4 强警示**复用既有持仓提醒通道 `CATEGORY_D5EXIT`**(语义泛化为「持仓需处置」= D5 时间退出 ∪ K4 派发警报),受既有 `push_d5exit` 开关(或 builder 加 `push_holding_alert` 开关解耦,开关非推送类不破「五类」);**APNs category 仍五类**。**用户可否决**:若要 K4 强警示独立开关 / 独立 category → 升级第六类(须批准打破「五类」)。落地 §五 v1.3-②。守 §2.4 铁律「证伪只用价量结构」:换手/年线下涨停/放量大阳=强价量证据、题材天数=成分弱证据标「参考」。
...
- **③-C3 候选池四步管线改版(需求 5,K1 entry mask 退役)**:新 `report/intel_candidates.py`(或改造 `candidates.py`——但**候选评分 `_base_score_expr` 保留供自选体检/纪律红绿灯复用,不删**):
  - **① 板块层** = 主线识别器拥挤度 top(`compute_sector_strength`):**用户五板块常驻**(科技/药/电/机器人/材料,存 `app_settings.intel_watch_boards` 可配、映射到具体概念板块 code)+ **当日暴起板块**(当日拥挤度 top-N)。
...
6. **章程 staged 激活实证(用户)**:清仓 + 确认后跑切换器 → `is_active=v1.3`、回落 8% + 两档时间退出 + 4 万区间生效、下一份周复盘用 v1.3 判激活后周 / K1 判之前周(洗白修复实证);**未误激活 v1.2**。
...
- **K4 持仓牌(②)**:`PositionOut` 内嵌 `k4Advisory:[{code, label, level(strong/normal), evidence, evidenceStrength(price_volume/constituent)}]`(读 DB advisory 命中,服务端算好);强警示走既有持仓提醒 APNs(`CATEGORY_D5EXIT`)+ 既有开关(或新增 `push_holding_alert` 字段于 `GET/PUT /settings/push`——**开关非推送类,不破五类**)。
...
- **[✅ 已进 v1.3 施工图 §五 v1.3-③「情报官」,2026-07-26 立项] 盘前情报包(需求 3,2026-07-25 交接·最新版清单)**:两部分——
