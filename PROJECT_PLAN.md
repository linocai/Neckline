...
**2026-07-20 阶段 1(策略研究)完工。** §6 参数清单 P1–P10 逐项过堂,产出「策略规则 v1」并版本化落 SQLite 大脑
(`strategy_versions.v1`,active)。完整方法/数据窗口/结果表/结论见 `research/stage1_report.md`(逐节)。全量 pytest 全绿。

**核心结论(诚实)**:**日线 2–5 日持有窗口内 A 股呈均值回归**,母战法"追强势短线"内核在此频率**无正 net edge**——
P3 强势定义三候选全否决、P4/P5 禁买规则全否决(被禁那批反而更会反弹)。规则 v1 因此是一套经验证的**减损纪律系统**
(strength=none + 回调低吸 + **-5% 止损** + **hold=5** + 回落止盈 5% + 主板 only + 仓位 2 万/5 只/60% 敞口):**样本外
(2025-2026)大幅跑赢 dummy 基准(-10.7% vs -65.3%)、把裸奔式短线的 -65%~-99% 爆亏压成 -11% 慢渗,但盈利因子 0.814<1
(仍净亏)**——**验收半达标,如实报告,不为凑 PF>1 过拟合**。两个用户拍板项:① **MA20 市场过滤器(P1)——实时闸门样本
内外双双变差,建议不采纳**(分层好看但滞后,条件均值≠可交易规则);② **次周单笔减半(P10)——空效应**(几乎不触发)。

**下一步**:阶段 2 盘后报告管线(§2.3,同码复用 rule v1 信号)。**开放的 alpha 问题**留待阶段 2+:更快的情绪信号
(情绪仪表盘领先指标)、资金面(moneyflow)、LLM 消息面审判(回测盲区)。研究期用户继续按 §2.1 纪律章程**手动**实盘。

**⚠ 高危区提示(建议 review)**:阶段 1 触及回测引擎撮合/复权(engine qfq 注入)与母战法退出/止损/仓位纪律(momentum.py)
= 资金逻辑高危区。虽有单测覆盖(momentum 19 + engine 注入等值 1),建议在进入阶段 2 前叫一次 `review` 独立复审这两块。
