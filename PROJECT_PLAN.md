...
**2026-07-19 阶段 0(数据层 + 回测引擎骨架)完工。** 0.1–0.8 全部交付,验收标准逐项达成,详见变更日志。仓库现状:`neckline/` 主包(config/calendar/data/backtest/strategy)+ `scripts/`(init_calendar/backfill/daily_update)+ `tests/`(100 个单测全绿)。全量历史数据已 backfill 落地(`data/parquet/` + `data/neckline.db`,均 gitignored)。

**下一步**:阶段 1 策略研究(§6 参数清单 P1–P10 逐项过堂),或先按需要小修 0.4b 已知遗留项(见变更日志「遗留问题」)。研究期用户按 §2.1 纪律章程**手动**实盘。
