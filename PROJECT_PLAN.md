# Neckline · V2.5.2 Build 15 控制面

> 当前生产：V2.5.2 · Build 15
> 现行引擎：K9 ｜当前阶段：10:00 macOS 可见结算快修、换包与真实界面验证完成
...
- Build 15 只修客户端结算展示路径：不改后端、策略、报告、通知或生产数据，不重跑报告。
...
- Build 15 保留上海时间 09:59:55–10:06:00 的只读结算终值同步；macOS 在当日结算完整后于核对表显示明确完成提示和直达入口，成绩页默认展示“三分支终值”，手动刷新选股也会补查终值。
...
- Build 15 的 macOS / iOS Release 签名归档均校验为 `2.5.2 (15)`，交付目录为
  `/Users/linotsai/Lino/releases/Neckline/v2.5.2-b15-20260824/`。iOS 与 macOS 压缩交付物
  SHA-256 分别为 `036c4c9c51fa111ac41d1ec262e2468356723b45e264c83810004062c40051cb` 和
  `37544a9379a22d43d3bbe779419e82c0379a53b3ef5d65729f65d4593ba8cae4`。
...
- 宁波云公开健康检查为 `v2.5.2`，服务 active、`NRestarts=0`；28 张表及逐表行数发布前后
  ...
  既有 `20260823` 报告仍可读取。
- macOS 已换装并启动 `2.5.2 (15)`，签名有效、归档与安装后可执行文件逐字节一致。
  真实界面已确认核对表显示“10:00 结算已完成”，直达入口可进入 2026-08-24 三分支终值，
  显示成立 11、放弃 3、观察 6、未定案 0。旧 Build 14 备份位于
  `/Users/linotsai/Lino/app_backups/Neckline-v2.5.2-b14-pre-v2.5.2-b15-20260824-105332/`；
  iOS `2.5.2 (15)` 签名归档已交付但未安装。
