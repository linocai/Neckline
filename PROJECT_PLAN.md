...
15. 备份使用通用 S3 对象存储与加密增量：每天 02:30 及每次发布前执行，保留 30 份日备份和 12 份月备份；解密私钥只在用户 Mac。
...
- 备份在未配置 S3、公开密钥、保留开关或隔离恢复验证器时 fail-closed；实际生产配置、首次异机恢复演练及 timer 启用均留到部署前授权。
- 第四轮独立复审已核验：恢复命令不触发报告/APNs，备份对象与密钥边界、legacy 删除、客户端 Keychain/HTTPS 与离线边界均通过；实际 S3 配置和首次异机恢复演练仍是部署前操作。
...
- **行为**：SQLite 在线一致性备份 + 冻结事实 parquet + 必需运行配置清单，生成校验 manifest，加密增量传至通用 S3；每天 02:30 及每次发布前执行，保留 30 份日备份和 12 份月备份。备份成功只在远端校验、可在隔离目录恢复并通过完整性检查后成立；服务器仅持加密所需公开材料，解密私钥只在用户 Mac，日志一律脱敏。
- **验收/回滚**：本地伪 S3 覆盖上传失败、校验失败、恢复成功、私钥不在服务器与保留策略；生产先做一次只读恢复演练再启用 timer。S3/加密配置缺失时服务拒绝启用而非退回本机假备份；回滚为停用备份单元，原数据不删。
...
日历不近似、备份可恢复、legacy 无残留、APNs 删除只针对永久失败。部署前才在已确认目标的只读备份上
做完整性/迁移审计和 restore rehearsal，检查 systemd 语法及 next trigger；此版本不执行部署。
