"""放量倍数 —— 全仓唯一实现(K9-v2 §3.0.1)。

> **放量倍数 = 当日成交量 ÷ 前 N 个交易日平均成交量**(N = `params.volume.maDays`)

P1 与 P2 使用同一个机械读数，但门槛各自独立标定：

| 用它的地方 | 判据 | 门槛 |
|---|---|---|
| P1 放量启动 | 定义性：放量倍数达到本档门槛 | `channels.p1.<档>.minVolMultiple` |
| 形态 2 超跌反弹 | 定义性「当日有实际换手」:放量倍数 **≥ 门槛** | `channels.p2.<档>.minVolMultiple` |

P3 不使用该读数划分身份；P1/P3 由“近期是否持续热门”机械分界。

⚠ **它不是 P4 的「量比」。** 量比的分母是前 **5** 个交易日均量(盘后口径,
