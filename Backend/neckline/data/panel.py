...
前复权拼接、日线、`daily_basic` 与涨跌停派生列在这里合流，只描述行情事实，
不决定 K10 的机会、公司比较或用户选择。K10 的固定 D1/D2 观察使用独立冻结
窗口与日事实；研究、回测和校准不在本仓库运行。

所有滚动和 ``shift`` 特征只读取当前行及更早的数据。``fwd_*`` 列仅供离线事实
研究，K10 运行时不得将其作为发现、比较或评价输入。
"""
