"""申万 2021 版行业分类：拉取、落库与校验。

这是事实层取数件，只回答「这只股票属于哪个申万二级行业」。它不包含强度、排名或
成员数等策略判断。K10 以稳定的二级代码识别白酒Ⅱ硬排；分类缺失时应如实标为资料不足，
不能按行业名称或题材猜测。
...
def verify(db_path: Optional[Path] = None) -> List[str]:
    ...
    **代码**不存在则是真问题 —— K10 白酒硬排按代码走，代码没了就无法可靠排除。
    """
    problems: List[str] = []
