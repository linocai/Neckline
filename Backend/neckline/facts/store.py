...
class FactPack:
    ...
    @property
    def path(self) -> Path:
        return day_file_path(PARQUET_TABLE, self.trade_date, self._parquet_dir)
...
def freeze_pack(
    ...
    *,
    ...
    try:
        ...
        final_path = day_file_path(PARQUET_TABLE, pack.trade_date, root)
...
def load_pack_range(
    ...
    **三个必填关键字,每个都是一道闸**(⛔ 都不给默认值,⛔ 都是抛不是警告):
    ...
    for day_s in days:
        ...
        p = day_file_path(PARQUET_TABLE, d, parquet_dir)
...
def trim_parquet(
    *,
    ...
    doomed = frozen_days[:-keep] if len(frozen_days) > keep else []
    removed: List[date] = []
    for d in doomed:
        p = day_file_path(PARQUET_TABLE, d, parquet_dir)
        if not p.exists():
