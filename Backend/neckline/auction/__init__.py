...
# ── 「竞价强势股不是全市场排行」的诚实披露(§五 ⑨-B-2,**恒发**)—————————————
# 🔴 **2026-08-12 用户裁定 ① 之后这句话换了主语**:取样域不再是盘中关注池,而是竞价层
# **自己的独立观察池**(同一主驱动 → 同题材方向〔本版未实现〕→ 传统行业,完整取样、
# 不设 29 只上限;唯一实现 `auction/observation.py`)。裁定原文要求「明确标注观察范围,
# **不冒充全市场排名**」—— 故本常量只讲**这一句不变的话**,**当天那份具体范围**
# (多少只、来自哪几层)由 `ObservationPool.scope_note` 随报告下发,⛔ 不写死在这里。
...
AUCTION_PROXY_SAMPLE_NOTE = (
    "竞价强势股只在系统当天的「竞价观察池」里排序,不是全市场竞价排行;"
    "观察池按 K8 的比较域顺序取数(同一主驱动 → 同题材方向 → 传统行业),"
    "与盘中关注池是两份互不相干的样本。它们只解释资金方向,不取得交易资格。"
)
