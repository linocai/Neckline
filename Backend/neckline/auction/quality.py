...
⛔ 它**不回答任何市场问题**(那不是本层的活;K9 的核对表只做条件求值),
⛔ 不做 IO、不碰 DB、不调 LLM —— 全是纯函数,`(读数, 交易日, 抓取时刻) → 判定`。

**为什么单起一个文件**:`collect.py` 的身份是「组清单 → 拉一次价 → 冻结,⛔ 不判定」,
而七项校验确实是一种判定;`checklist.py` 的身份是「代入 D0 冻结预案求值」,校验又不是求值。
...
🔴 **零新阈值**(§五 〇b-1;审计规格 P2.1 逐字「不得发明任意『5 分钟新鲜度』之类的新阈值」)
  ...
  · 时间判据只用制度已规定的**交易日**与 **9:25 / 窗口**边界;
  · 冲突判据的三类全是**结论性**的(方向相反 / 放弃分支触发与否 / 身份不一致),
    ⛔ 一个百分比都没有;
...
from neckline.auction import (
    AUCTION_RESULT_TIME_START as _AUCTION_RESULT_TIME_START,
...
#: ⚠ 唯一的**非致命**项是 `open_price_missing`:9:26 那一拍**本来就还没有开盘价**,
#: 而求值器对读不到的量本来就有第三态(`Truth.UNKNOWN` → 落「观察」)—— 把它算致命,
#: 等于因为一个当时用不上的字段把好端端的竞价价一起扔掉。见 `QF_DEGRADED` 那段说明。
...
def _gap(q: Quote) -> Optional[float]:
    """竞价涨跌幅(**百分点**)。⚠ 公式的唯一源在
    `playbook/model.py::gap_percent_points`,这里**只是调它**
    (⛔ 不在本模块抄第二份 —— 那正是两处容差各自漂移的复发路径)。
    ⚠ 本函数只用来判**方向**(涨 / 跌),量纲不影响判据。"""
    from neckline.playbook.model import gap_percent_points

    return gap_percent_points(getattr(q, "price", None), getattr(q, "pre_close", None))
...
def detect_conflict(
    ...
    *,
    ...
    if rejection_of is not None:
        a, b = rejection_of(primary), rejection_of(backup)
        if a is not None and b is not None and a != b:
            return CONFLICT_REJECTION_DISAGREE
...
class QuoteQuality:
    ...
    def status(self) -> str:
        ...
        (压根没有时间戳可解析),而这一位会顺着核对表进 `k9_d1_verdicts` 的读数账 ——
...
def resolve_dual(
    ...
    *,
    ...
    if _is_cross_verified(checks):
        conflict = detect_conflict(primary, backup, rejection_of=rejection_of)
...
def domain_quality(
    ...
    ⚠ 与 `collect.Snapshot.quality_of` 是**同一套判据的两个入口**:那个吃快照,
    这个吃已经算好的逐票账(核对表侧拿它算「这批票今天的读数整体够不够用」)。两者由守门单测对拍。
    """
