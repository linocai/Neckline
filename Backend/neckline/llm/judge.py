...
def judge_candidate(
    ...
    *,
    ...
    `system_prompt`(v1.1-C.3 新增,默认值不变,**调用点零改动、纯向后兼容扩展**):
    调用方可传自己的框定语,只要结尾仍用同一个"结论:通过|否决"机器可读标签,
    `_parse_verdict`/降级链/`JudgeResult` 结构即完全共用,不必重写任何解析或降级
    逻辑。⚠ **V2-⑬-11**:V1 唯一的自定义 prompt(自选体检
    `WATCHLIST_JUDGE_SYSTEM_PROMPT`)已随自选池整链删除;**参数本身保留** —— ⑬-2
    明确 `judge_candidate` 本体是「通用 LLM 调用 + 降级链 + verdict 解析」工具。
