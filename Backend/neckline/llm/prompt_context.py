...
**为什么单起一个模块**:最初三条 LLM 链路——问询台(`api/inquiry.py`)、候选/自选审判
(`llm/judge.py`)、参考件三件套(`report/reference_plan.py`)——需要的是**同一件**东西:
告诉模型"现在是哪一天"、以及"引用材料必须带日期"。**只此一份实现**,各处 import;
**不许各抄一份**(抄三份 = 三种日期口径,正是本次故障的同类病)。⚠ 上述"最初三条"
里的问询台(V2.1-①)、参考件三件套(V2.0.0-⑬-3)均已随各自那波退役整链删除;
**现役消费方名单**(会持续随新链路增减,不在此手动重复维护)以
`tests/test_prompt_context.py::TestTimelinessRulesAreInEveryPrompt` 的参数化清单
为准——那份清单本身就是机器可查的当前事实。
