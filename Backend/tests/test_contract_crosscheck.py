...
文件零 `/api` 字面量)——改为扫 `App/` 下**全部** `.swift` 文件,日后任何 View/
...
_CLIENT_DIR = _ROOT.parent / "App"
_API_CLIENT = _ROOT.parent / "App" / "Neckline" / "Networking" / "APIClient.swift"
...
def _normalize(path: str) -> str:
    ...
    `{}`(🔵-2 小审 2026-08-03:`App/NecklineTests/URLGateTests.swift` 用
...
def client_call_surface() -> set:
    ...
    **🔵-2 小审 2026-08-03 起扫 `App/` 下全部 `.swift` 文件**(含 `NecklineTests/`),
...
def test_retired_push_kind_flag_is_declared_on_both_sides():
    ...
    hits = []
    for path in sorted((_ROOT.parent / "App" / "Neckline").rglob("*.swift")):
