...
`degraded=True`(不是"确认无消息",也不是硬造某个类别命中);⑤ 与通用 Provider +
MockTransport 端到端。"""
...
class TestVerdictParsing:
    ...
    def test_single_category_hit_parsed(self):
        ...
        stub = _StubProvider(LLMResult(ok=True, content=content, provider="custom", model="test-model"))
        r = scan_news_for_code("600001.SH", "示例甲", provider=stub)
        assert r.degraded is False
    ...
    def test_missing_format_is_degraded_not_confirmed_clean(self):
        ...
        stub = _StubProvider(LLMResult(ok=True, content="模型只写了一段话,忘记按格式收尾。", provider="custom", model="test-model"))
        r = scan_news_for_code("600001.SH", "示例甲", provider=stub)
        assert r.degraded is True
...
class TestEndToEndWithRealProviderMockTransport:
    def test_provider_end_to_end_hit(self):
        ...
        provider = OpenAICompatProvider(
            api_key="sk-xxx", model="test-model", name="test-provider",
    ...
    def test_provider_end_to_end_none_found(self):
        def handler(request: httpx.Request) -> httpx.Response:
            return httpx.Response(200, json={
                "id": "x", "model": "test-model",
                "choices": [{"finish_reason": "stop", "message": {
