...
def snapshot_env(isolated_env):
    """临时库 + 临时 parquet 根,铺三天事实包 parquet 与对应的 `fact_packs` 清单行。

    ⛔ 全程不碰工作库 / 工作 parquet(AGENTS.md 测试纪律)。"""
    from neckline.facts.pack import PACK_VERSION

    root = isolated_env.parquet_dir / "fact_pack" / "year=2026"
    root.mkdir(parents=True, exist_ok=True)
    days = ["20260102", "20260105", "20260106"]
    for d in days:
        (root / f"{d}.parquet").write_bytes(b"PAR1-fixture-" + d.encode())
    ...
    try:
        for d in days:
            conn.execute(
                ...
                (f"pid-{d}", d, PACK_VERSION, "live", "frozen", "sha", 5526,
                 "{}", "{}", 0, "t"))
        conn.commit()
    finally:
        conn.close()
