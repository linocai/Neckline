...
#:
#: ⚠ **这是一份黑名单**:它拦的是「把旧东西照原名接回来」,拦不住「换个名字重做一遍」。
#: 那一半由**允许面**判据顶着 —— `AppTab` 恰好四个成员(组 B)、该有的文件清单
#: (组 C)—— 两者合起来才是「三板块 IA」这句话的判据。
#:
...
#: ⚠ **这一半是黑名单**(上面的「允许面」那一半才是白名单)。会漏:有人把终值换个
#: 名字搬运(`let settled = ...` 再传进选股屏)、或者只搬**取值**不搬类型。
#: ⛔ 不能简单粗暴地禁词根 `verdict` —— `ChecklistVerdict` 是 9:29 那一拍的**二值**
#: 枚举,它天经地义地住在核对表面上,禁词根会把它一起判红,而误报会逼着后来者
#: 把守门放宽。两侧的取舍就落在这里,写清楚免得下一个人以为是漏网。
...
#: 直白的合计口径命名(大小写不敏感)。
#: ⚠ **这是一份黑名单,它一定会漏下一种** —— 取个中性名字(`nkSum`、`combine`、
#: `overall`)就绕过去了。真牙齿是下面那个 `_combining_expressions`(形状判据:
#: 两个分数出现在同一个算术表达式里),它不问你把函数叫什么。
#: 黑名单留着只是因为**命名本身也是一种承诺** —— 一个叫 `combinedScore` 的字段,
#: 哪怕现在没人算,下一个人也会去填它。
_COMBINED_NAMES = (
