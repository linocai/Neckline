...
def test_selected_debate_then_morning_contrary_withdraws_without_erasing_history(tmp_path, monkeypatch):
    ...
    store.enqueue_task(task_id="morning-task", kind="morning_review", idempotency_key="morning", input_version="v", input_cutoff_at=morning_cutoff, payload={"candidateId": "cand-1", "observationId": chosen["observationId"], "originalCutoffAt": NOW, "morningEvidenceRefs": [{"documentId": "doc-2", "revision": 1}], "sourceStatus": "complete", "configId": "cfg", "configRevision": 1}, budget={"maxAttempts": 2}, created_at=morning_cutoff, db_path=path)
    morning = FakeProvider([_result('{"material":true,"reasonStatus":"invalidated","observationStatus":"needs_review","summary":"晨间反证","materialContraryEvidence":[{"documentId":"doc-2","revision":1,"claim":"重大反证"}]}')])
