...
def test_selected_debate_then_morning_contrary_withdraws_without_erasing_history(tmp_path, monkeypatch):
    ...
    run_once(db_path=path, worker_id="morning", lease_for=timedelta(minutes=5), handlers={"morning_review": morning_review_handler}, clock=lambda: datetime(2026, 9, 7, 1, tzinfo=timezone.utc), require_b76_contract=True)
