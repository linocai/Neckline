...
def test_evening_excludes_exactly_2100_and_morning_includes_both_boundaries():
    ...
    at_0830 = datetime(2026, 9, 7, 8, 30, tzinfo=SHANGHAI)
    assert evening.contains(at_2100) is False
    assert morning.contains(at_2100) is True
    assert morning.contains(at_0830) is True
    assert morning.contains(at_0830 + timedelta(microseconds=1)) is False


def test_morning_cutoff_is_fixed_even_if_task_runs_late():
    assert morning_cutoff(date(2026, 9, 7)) == datetime(2026, 9, 7, 8, 30, tzinfo=SHANGHAI)
