#!/usr/bin/env python3
"""盘后**篮子日报** CLI(plan §五 V2-⑭-A)。只跑「报告」那一段:读**已经冻在库里的**
东西(情绪 / 板块 / 持仓体检 / 情报 / 资金流 / 消息面 / 篮子四表 / 复盘表)→ 渲染五段
markdown → 落文件 + SQLite 存档。

⚠ **本脚本不批算扫描层、不生成篮子、不跑复盘**。那条 16:35 链在 `scripts/evening.py`
(⑧ 验证 → ④ 扫描 → ⑤⑥⑦ 篮子 → ⑨ 复盘 → 报告)。**只跑本脚本的后果**:③b「今日未定档
篮子」会如实标「本段未取得」(⑥ 的溢出篮不进表,只随那一次编排走),③/④ 两节读的是库里
已有的东西。历史回放(§2.6)要的正是这个只读姿势 —— ⛔ 回放一份三个月前的报告绝不能
顺手往篮子表里写今天算出来的东西。
