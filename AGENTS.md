...
- Production is **K10-v2 / Neckline 3.2.0 Build 65 / Schema 8**, backend release set **v3.2.0-b65**, since 2026-09-10. K9 and the earlier K8 chains are retired from active production and this worktree; do not reintroduce their runtime code, tables, routes, settings, compatibility shims, or UI placeholders. The explicit offline V3 migration owns the legacy deletion boundary; Git history is the archive.
...
- **Current production state (2026-09-10 B65 continuation):** B65 `49a2974` is deployed in Ningbo; API active+enabled, run control open. At 23:16:05 the official CLI resumed the original task `task_c9c0feab5c83a08e8a17138ebfb0041a` / scan `scan_d39fbd5c631ea9a7df5f569dbd432b4d`; all 326 completed checkpoints and billing records are unchanged. Dedicated `neckline-trial-20260910-evening-b65.service` continues this task and its successful report notification only; normal worker/timers and old tasks remain paused. User requires the complete report, readable output and actual push before ending. BYOK deepseek-flash. Rollback set `/opt/neckline/data/backups/v3.2.0-b65-predeploy/` preserves B64 runtime/current settings/data; never overwrite later writes. Mac remains B61 while locked, B65 staging and B61 backup verified; iOS archive ready for Xcode. See PROJECT_PLAN.md / execution record section 30.
- **Current released baseline:** 3.2.0 / both platforms Build 65 (B58–B64 fixes plus B65 comparison scope filtering and paid-response reuse), following the latest `/Users/linotsai/Lino/whynotme/K10.md` (K10-v2). The fixed 1,089-company universe, removal of 80/40 full-text caps, labeled-unverified recommendation path and daily company selection supersede conflicting v1.4 implementation rules. Existing v1.4 publication evidence remains historical, not a new-version behavior contract.
