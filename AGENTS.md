...
- **Current production state (2026-09-13):** 3.2.1 backend B64 `fa94f81` is deployed in Ningbo. The original September 10 21:00 task `task_c9c0feab5c83a08e8a17138ebfb0041a` / scan `scan_d39fbd5c631ea9a7df5f569dbd432b4d` remains completed: 7 cards, 22 research events, publication September 11 01:06:26; APNs accepted both devices at 01:06:28. Preserve partial upstream coverage and unknown market acquisition times. Do not recover/replay the report or resend its notification. API and normal worker are active+enabled, run control open; morning/evening and market-update/retry timers are active+enabled. Notifications are ready. The sole pre-cutover queued analysis was cancelled before its first attempt; old failed/completed tasks are not replayed. New rollback `/opt/neckline/data/backups/v3.2.1-b64-predeploy/` preserves B63 and all current writes; never overwrite later writes. Mac 3.2.1 B62 is installed, strictly signed and reading the original report; iOS signed archive is ready for user installation through Xcode. See `archive/v3.2.1-b59_execution.md` section 17.
...
- **Provider authorization boundary (2026-09-13):** User now authorizes routine production, including scheduled reports, normal analysis, market/evaluation maintenance and notifications. Morning runs at 09:00 Asia/Shanghai on each exchange trading day; evening runs at 21:00 on the immediately preceding natural day. This supersedes the old routine-scheduling pause. Isolate pre-cutover queued work; never replay the completed September 10 report or resend its notification. Do not make unrelated provider/balance/permission probes. Tests remain isolated/network-denied; profiles retain draft/provenance status.
