...
- Production is **K10-v2 / Neckline 3.3.0 release clients Build 69 / backend Build 70 / internal Schema 9 (public report Schema 8)**, backend release set **v3.3.0-b70**, since 2026-09-15. K9 and the earlier K8 chains are retired from active production and this worktree; do not reintroduce their runtime code, tables, routes, settings, compatibility shims, or UI placeholders. The explicit offline V3 migration owns the legacy deletion boundary; Git history is the archive.
...
- **Current production state (2026-09-15 18:50 CST):** 3.3.0 backend B70 `2e7f22f` is deployed in Ningbo. Mac B69 is installed and verified; iOS B69 is signed/Xcode-ready, with physical device installation left to the user. API and all four timers are active+enabled; notifications ready. Due to confirmed Tavily HTTP 432 plan-limit rejections, run control is temporarily **closed** (`tavily_plan_limit`) and the gracefully stopped worker is inactive+enabled, preventing new paid model work before unusable verification. The original report deadline has now expired. Quota restoration and disposition of that expired task must be resolved before paid recovery; do not automatically reopen control, extend the deadline, or create a replacement. The approved timetable remains unchanged. Old September 10 and earlier reports were explicitly deleted at September 13 19:51; never restore/replay them or resend notifications. Configuration, company snapshots, source progress, market data and paid/delivery audit remain intact. All four configuration scopes are ready while run control correctly remains paused. The September 15 09:00 scheduled morning entry correctly skipped the closed control with exit 0 and no new task. See `archive/v3.3.0-b69_execution.md` section 14; original incident evidence remains in the prior record section 17.
- **September 13 evening incident:** Original task `task_00437ea0ac27c8601b8f5711494dc801` / scan `scan_0cfbef8493c21a2d0ed62bee9992caf5` is failed at attempt 42, stopped at 23:49:36 after Tavily quota refusal. Of 86 events, 68 reached final research states, 3 have legitimate pending-verification conclusions, and 15 failed verification execution. All 30 title batches, 2 reconciliations, 57 bodies and 23,633,932 recorded model tokens remain. B67's original reread bug is fixed and passed in production. B68 recognizes 432/433 as quota failures and gives definitively failed verification one count/input-bound retry only through explicit same-task recovery; ordinary restart cannot renew it. Original frozen input, paid history and deadline (September 14 03:00:13 CST) remain unchanged. No report/cards published; latest failure push accepted for both devices. Await confirmation of usable Tavily quota and an explicit disposition of the now-expired original task; preserve its identity, window, budget, deadline and ledger, and never create a replacement automatically. See section 17.
- **Current released baseline:** 3.3.0 / both client releases Build 69 / backend Build 70. Research uses action-specific visible evidence, versioned local reads, shared source facts and interruption-safe query closure; recommendation roles and historical identity are separated. B71 price-reference DTO and prior recovery/fulltext fixes remain included. Follow `/Users/linotsai/Lino/whynotme/K10.md`: fixed 1,089-company universe, no 80/40 fulltext caps, labeled-unverified recommendation and daily company selection.
- **Provider authorization boundary (2026-09-13):** User now authorizes routine production, including scheduled reports, normal analysis, market/evaluation maintenance and notifications. Morning runs at 09:00 Asia/Shanghai on each exchange trading day; evening runs at 21:00 on the immediately preceding natural day. This supersedes the old routine-scheduling pause. Isolate pre-cutover queued work; never restore/replay the user-deleted September 10 report or resend its notification. Do not make unrelated provider/balance/permission probes. Tests remain isolated/network-denied; profiles retain draft/provenance status.
...
- After the 2026-09-15 B70 release, retain only `v3.3.0-b69-predeploy` and `v3.3.0-b70-predeploy`, restoring B68/B69 code respectively. Verified gzip snapshots and runtime/evidence total 315,280,657 bytes (approximately 0.294 GiB). B70 pre/post share one inode; B69 runtime is verified against current Schema9 for code-only rollback without restoring the database. B69 pre/post retain Schema8/9 separately: narrow 9→8 rollback requires stopped writers, closed control, no unresolved calls and zero private receipts; nonempty receipts prohibit automated downgrade and require a forward repair. Never overwrite new business writes with an old DB. Superseded B68 recovery set and this release's temporary uploads/rehearsals were removed after verification.
- The report-cleanup recovery snapshot is `/opt/neckline/data/archive/report-cleanup-20260913/pre-cleanup.db.gz`; retain only until the first post-cleanup scheduled report completes and is readable, then remove this cleanup-only snapshot. After service restoration, never restore this whole DB over new writes; any recovery requires a reviewed, scoped restoration. B69/B70 release rollback sets remain separate.
