...
- Production runs **K10-v1.4 / Neckline 3.1.0 Build 51 / Schema 7**, backend release set **v3.1.0-b51**, since 2026-09-09. K9 and the earlier K8 chains are retired from active production and this worktree; do not reintroduce their runtime code, tables, routes, settings, compatibility shims, or UI placeholders. The explicit offline V3 migration owns the legacy deletion boundary; Git history is the archive.
...
- **Current production state (2026-09-09 00:42 CST):** B51 is deployed; the same user-authorized September 8 evening task resumed with all 67 selected bodies and 13 supplemental bodies preserved. Resume closes a persisted assessment before new query planning; exhausted fulltext capacity blocks new requests, and denied empty results do not trigger repeated assessment. Research semantics are validated before completed caching; rejected historical derivatives remain auditable. Switch open and worker active, evening/morning timers await final report verification. Current task keeps execution revision 2 plus recorded runtimeRepair (32768 investigation output tokens; original start +21600 seconds); future schedules bind revision 3. Never recover the retired 2,472-article incident batch. See `PROJECT_PLAN.md`.
