...
- Production is **K10-v2 / Neckline 3.3.0 release clients Build 69 / backend Build 75 / internal Schema 9 (public report Schema 8)**, backend release set **v3.3.0-b75**, since 2026-09-16. K9 and the earlier K8 chains are retired from active production and this worktree; do not reintroduce their runtime code, tables, routes, settings, compatibility shims, or UI placeholders. The explicit offline V3 migration owns the legacy deletion boundary; Git history is the archive.
...
- **Local 3.5.0 / client Build81 frontend quickfix / backend Build80 (2026-09-21):** Follow the current Plan and `archive/v3.5.0-b78_execution.md`. User authorized local implementation/offline acceptance only; no production/provider/APNs calls, deployment, timer restart or old-task recovery. New morning starts 08:30 with a 09:20 readable-result deadline; new evening starts 21:00 without a whole-report business deadline. This supersedes the old 09:00 source schedule for new B78 tasks, not historical task bindings or the production scheduling pause. Both clients upgrade together; historical reports/receipts remain readable. Unranked completed materials are read-only and never formal recommendations or D1/D2 samples.
- **Current production state (2026-09-16 03:00 CST):** B75 `0a834a9`已部署，客户端B69。9月15日晚报原task于01:19:12在第81次执行停止：84事件中59比较完成、16背景、8合法待核，仅机器人合资事件因content_policy_refused失败；最终排序已付费完成。实际报告API为failed/availableAt空/0卡，失败通知APNs已接受2设备令牌，未发送完成报告通知。1,281完成检查点保留，已记19,039,904 token（4次拒绝usage未知），Tavily396请求/310实报credits；无在途调用、无重复成功wire。03:00:33只读确认原03:00:18期限已过，报告仍failed且无新增调用/推送；用户部分发布决定仍未答复。禁止自动恢复、部分发布、切换冻结模型、延期或创建替代任务。 2026-09-16 09:21 CST用户要求关闭所有定时：四个Neckline timer均inactive+disabled、无下一触发时间；Codex心跳neckline已PAUSED。API/worker保留，control未改；未获新指令不得恢复定时。 See `archive/v3.3.0-b69_execution.md` section 16.5.
- **Handoff override (2026-09-16 09:40:59 CST):** All four timers and Codex monitoring remain off, but the already-started morning task `task_af6fdc37bdd143b4d69e4f7b4d1825e4` is still running (6,205,300 recorded tokens / Tavily104 credits / one model call in flight at check). Read live state first; do not claim all work or spend stopped. User requested analysis then handoff, not a new implementation or schedule restart. See current Plan and archive section16.7.
