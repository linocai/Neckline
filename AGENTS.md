...
- Production is **K10-v2 / Neckline 3.2.0 Build 60 / Schema 8**, backend release set **v3.2.0-b60**, since 2026-09-10. K9 and the earlier K8 chains are retired from active production and this worktree; do not reintroduce their runtime code, tables, routes, settings, compatibility shims, or UI placeholders. The explicit offline V3 migration owns the legacy deletion boundary; Git history is the archive.
...
- **Current production state (2026-09-10 B60 recovery):** B60 `03fb861` is deployed in Ningbo, API active+enabled. The user authorized correcting brittle pool validation and continuing tonight’s same frozen report. At 21:25:47 the official recovery CLI resumed `task_c9c0feab5c83a08e8a17138ebfb0041a` / `scan_d39fbd5c631ea9a7df5f569dbd432b4d`; dedicated `neckline-trial-20260910-evening-b60.service` is running. Run control is open; normal worker and all timers remain inactive+disabled. The original 22 paid title checkpoints were retained and validated offline; no new input collection or historical task recovery. BYOK remains deepseek-flash. Preserve September 8 report/choices/D1-D2 and failed September 9 morning report. Current rollback set `/opt/neckline/data/backups/v3.2.0-b60-predeploy/` preserves B59 runtime plus tonight’s data/settings; later writes must be checked before any restore. Bindings remain production rev 1 / execution rev 1. See PROJECT_PLAN.md and execution record section 25.
- **Current released baseline:** 3.2.0 / both platforms Build 60 (B58 SOP, B59 BYOK and B60 pool filtering fixes), following the latest `/Users/linotsai/Lino/whynotme/K10.md` (K10-v2). The fixed 1,089-company universe, removal of 80/40 full-text caps, labeled-unverified recommendation path and daily company selection supersede conflicting v1.4 implementation rules. Existing v1.4 publication evidence remains historical, not a new-version behavior contract.
- **Provider authorization boundary:** The user authorized this evening trial and, after the first failure, its B60 repair and same-task continuation. Only the frozen 2026-09-10 21:00 task may make required provider calls. No separate balance/permission probes, unrelated live validation, old task recovery or routine schedule resumption. Tests remain isolated and network-denied. Company profiles retain their original draft/provenance status.
