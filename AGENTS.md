...
- Production is **K10-v2 / Neckline 3.2.0 Build 57 / Schema 8**, backend release set **v3.2.0-b57**, since 2026-09-10. K9 and the earlier K8 chains are retired from active production and this worktree; do not reintroduce their runtime code, tables, routes, settings, compatibility shims, or UI placeholders. The explicit offline V3 migration owns the legacy deletion boundary; Git history is the archive.
...
- **Current production state (2026-09-10 release):** B57 is deployed in Ningbo; API is active+enabled for client reads. Durable run control stays closed; worker and all Neckline timers stay inactive+disabled. The release did not authorize a provider test, first report, schedule resumption or failed-task recovery. Preserve the September 8 evening report, original selections and fixed D1/D2 windows, and the failed September 9 morning report. Never replay the completed evening report or recover the retired 2,472-article incident batch. Rollback set: `/opt/neckline/data/backups/v3.2.0-b57-predeploy/`, preserving B53 runtime/database/binding; verify later writes before any restore. Current bindings are `k10-v2-production` rev 1 and `k10-v2-execution-production` rev 1. See `PROJECT_PLAN.md`.
- **Current released baseline:** 3.2.0 / both platforms Build 57, following the latest `/Users/linotsai/Lino/whynotme/K10.md` (K10-v2). The fixed 1,089-company universe, removal of 80/40 full-text caps, labeled-unverified recommendation path and daily company selection supersede conflicting v1.4 implementation rules. Existing v1.4 publication evidence remains historical, not a new-version behavior contract.
