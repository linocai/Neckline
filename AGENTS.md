...
- Production is **K10-v2 / Neckline 3.5.1 release clients Build 89 / backend Build 89 / internal Schema 10 (new public report Schema 9, historical Schema 8)**, release set **v3.5.1-b89**, since 2026-09-22. macOS is installed with launch verification pending Mac unlock; iOS signed archive is ready for the user to install through Xcode. K9 and the earlier K8 chains are retired from active production and this worktree; do not reintroduce their runtime code, tables, routes, settings, compatibility shims, or UI placeholders. The explicit offline V3 migration owns the legacy deletion boundary; Git history is the archive.
...
- **Current release (2026-09-22):** B89 `c9f1bb98ba4fd4edda627a422ce8534d38a5bb3c` is deployed under tonight’s quick-fix authorization; see `archive/v3.5.1-b82_execution.md` section16. September22 evening report published partial at23:53:11 CST with30 companies/58 events; completion APNs accepted for both devices at23:53:27/29. B89 removes the obsolete pre-publication diagnostic without rewriting the published report and preserves identical candidate facts across collection-clock-only retries. B83–B88 fixes remain included. No other old task is authorized. Production run/execution revisions are both2, bound to `k10-v2-b82-20260922`; old immutable bindings and paid rows are retained. New morning starts08:30 with09:20 readable-result deadline; evening starts21:00 with no whole-report business deadline. Both clients release together, old reports/receipts remain readable, unranked materials are read-only and never formal recommendations/D1D2 samples.
...
- 当前发布恢复集为B81/B82/B89（分别保留B75/B81/B88代码）。B89无DDL、配置不变，部署前后91表相同且B88回退API已验证；不得整库覆盖任何发布后的新写。B75的Schema9回退仍仅限原历史条件。回收证据见B82记录第16节。
- The cleanup-only snapshot `/opt/neckline/data/archive/report-cleanup-20260913/pre-cleanup.db.gz` was removed on2026-09-23 after the first new scheduled formal report was read through the API/current Swift and completion APNs verified. Superseded B83/B85/B86/B87/B88 recovery sets were retired after verifying B81/B82/B89. This does not delete business reports, paid ledgers or current release recovery.
