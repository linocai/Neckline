...
- Production is **K10-v2 / Neckline 3.2.0 Build 69 / Schema 8**, backend release set **v3.2.0-b69**, since 2026-09-11. K9 and the earlier K8 chains are retired from active production and this worktree; do not reintroduce their runtime code, tables, routes, settings, compatibility shims, or UI placeholders. The explicit offline V3 migration owns the legacy deletion boundary; Git history is the archive.
...
- **Current production state (2026-09-11 B69 continuation):** B69 `3ab7275` is deployed in Ningbo; API active+enabled, run control open. At 00:39:47 the official CLI resumed the original September 10 21:00 task `task_c9c0feab5c83a08e8a17138ebfb0041a` / scan `scan_d39fbd5c631ea9a7df5f569dbd432b4d`; all 806 completed checkpoints, billing and old tasks are unchanged. Dedicated `neckline-trial-20260910-evening-b69.service` continues only this report and its successful notification; normal worker/timers and old tasks remain paused. The requested September 10 deadline was missed; the report and actual push are still required before ending. BYOK deepseek-flash. Rollback set `/opt/neckline/data/backups/v3.2.0-b69-predeploy/` preserves B68 runtime/current settings/data; never overwrite later writes. Mac remains B61 while locked, B69 staging and B61 backup verified; iOS archive ready for Xcode. See execution record section 34.
- **Current released baseline:** 3.2.0 / both platforms Build 69. B69 preserves multiple model-authored tied groups and reuses completed fulltext only when historical hashes prove an explanation-only change; B67 recovery and B68 current stopping-policy fixes are included. Follow `/Users/linotsai/Lino/whynotme/K10.md`: fixed 1,089-company universe, no 80/40 fulltext caps, labeled-unverified recommendation and daily company selection. B67 assets exist but were never deployed separately.
