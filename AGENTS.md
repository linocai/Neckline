...
- **Current production state (2026-09-15 19:54 CST):** 3.3.0 backend B70 `2e7f22f` is deployed in Ningbo; clients remain Build69. User saved a new Tavily key and explicitly authorized one-company live verification followed by tonight's new scheduled report. The isolated smoke completed, its data and copied credentials were removed, and control reopened at 19:53 (`user_authorized_after_b70_live_smoke`). API, worker and all four timers are active+enabled; four configuration scopes and notifications ready. Next evening run is September 15 21:00 CST and next morning September 16 09:00 CST; no formal report for today existed at final check. The expired September 13 task remains failed and its ledger is unchanged: this authorization does not reopen it, extend its deadline or create a replacement for it. Old September 10 and earlier reports remain deleted; never restore/replay them or resend notifications. See `archive/v3.3.0-b69_execution.md` section 15.
