...
- Production is **K10-v2 / Neckline 3.5.1 release clients Build 88 / backend Build 88 / internal Schema 10 (new public report Schema 9, historical Schema 8)**, release set **v3.5.1-b88**, since 2026-09-22. macOS is installed with launch verification pending Mac unlock; iOS signed archive is ready for the user to install through Xcode. K9 and the earlier K8 chains are retired from active production and this worktree; do not reintroduce their runtime code, tables, routes, settings, compatibility shims, or UI placeholders. The explicit offline V3 migration owns the legacy deletion boundary; Git history is the archive.
...
- **Current release (2026-09-22):** B88 `fd490573d38c1a9e6dc72e11aefd634604174551` is deployed under tonight’s quick-fix authorization; see `archive/v3.5.1-b82_execution.md` section15. At23:51 the same September22 evening task resumed after two paid research results were revalidated with zero new calls. B88 ignores empty wire extensions and exact duplicated conclusion fields; persisted/evidence contracts remain strict. B83–B87 recovery/performance fixes remain included. No other old task is authorized. Production run/execution revisions are both2, bound to `k10-v2-b82-20260922`; old immutable bindings and paid rows are retained. New morning starts08:30 with09:20 readable-result deadline; evening starts21:00 with no whole-report business deadline. Both clients release together, old reports/receipts remain readable, unranked materials are read-only and never formal recommendations/D1D2 samples.
...
- 当前发布恢复集为B81/B82/B83/B85（分别保留B75/B81/B82/B83代码；今晚任务闭环前暂留）；压缩与回收证据见B82记录第8、10、12节。B85无DDL，部署前已验证同冻结绑定下B85/B83真实API；恢复后出现新写，不可整库回退，先核对冻结契约与未决费用，必要时前向修复。B75的Schema9回退仍仅限五张新表为空且旧数据不变的历史条件。清报告专用快照继续保留至首份新正式报告可读。
