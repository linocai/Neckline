...
- Production is **K10-v2 / Neckline 3.3.0 release clients Build 69 / backend Build 72 / internal Schema 9 (public report Schema 8)**, backend release set **v3.3.0-b72**, since 2026-09-15. K9 and the earlier K8 chains are retired from active production and this worktree; do not reintroduce their runtime code, tables, routes, settings, compatibility shims, or UI placeholders. The explicit offline V3 migration owns the legacy deletion boundary; Git history is the archive.
...
- **Current production state (2026-09-15 23:14 CST):** B72 `ee5c2de` 已部署并发布GitHub，客户端仍B69；23:13续做今晚原task，563个原完成检查点、付费账本、冻结输入及9月16日03:00:18截止均保留。混公司搜索计划已本地复用，拦截记录错误对应事件已恢复；另一个事件 `research_0c66f51015fc79526dc7be68b215dd05` 仍报 `investigation_execution_failed`，需继续定位。23:09受控暂停产生了双端未完成推送，是待修通知误报。报告和最终推送尚未完成，心跳每2分钟继续；Xcode暂停处理。 API/worker/four timers active+enabled, control open (`user_authorized_after_b70_live_smoke`). Same task/scan identity remains in PROJECT_PLAN.md; old September 13 expired task and deleted reports remain untouched. See `archive/v3.3.0-b69_execution.md` section 16.2.
...
- After the September 15 B72 release, retain only `v3.3.0-b71-predeploy` and `v3.3.0-b72-predeploy`, restoring B70/B71 code respectively on current Schema9. Verified recovery sets totaled 234,062,603 bytes at cleanup; B72 identical pre/post gzip snapshots share one inode. Superseded B70 set and exact release uploads/rehearsals were removed. Local B72 test/private-reply/build/fixture artifacts: 155,451,275 bytes → 0. Private receipts prohibit automatic Schema downgrade; never overwrite new writes with old snapshots.
- The report-cleanup recovery snapshot is `/opt/neckline/data/archive/report-cleanup-20260913/pre-cleanup.db.gz`; retain only until the first post-cleanup scheduled report completes and is readable, then remove this cleanup-only snapshot. After service restoration, never restore this whole DB over new writes; any recovery requires a reviewed, scoped restoration. B71/B72 release rollback sets remain separate.
