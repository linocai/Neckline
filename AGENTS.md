...
- Production is **K10-v2 / Neckline 3.3.0 release clients Build 69 / backend Build 73 / internal Schema 9 (public report Schema 8)**, backend release set **v3.3.0-b73**, since 2026-09-15. K9 and the earlier K8 chains are retired from active production and this worktree; do not reintroduce their runtime code, tables, routes, settings, compatibility shims, or UI placeholders. The explicit offline V3 migration owns the legacy deletion boundary; Git history is the archive.
...
- **Current production state (2026-09-15 23:48 CST):** B73 `0c90006`已部署并发布GitHub，客户端仍B69；23:47原队列继续，846个完成检查点、806条外呼审计、冻结输入和9月16日03:00:18截止均保留。双位置证据已成功保存并完整读回，原20,543 token评估仅有一次成功外呼；本次维护没有新增失败推送。还剩一事件HTTP400（两次拒绝，用量未知），详细上游原因旧记录未保存，需继续定位和受控续做。正式报告/最终推送尚未完成，心跳每2分钟继续；Xcode暂停处理。 API/worker/four timers active+enabled, control open (`user_authorized_after_b70_live_smoke`). See `archive/v3.3.0-b69_execution.md` section 16.3.
...
- B73发布恢复集仅保留B72/B73（恢复B71/B72代码），清理时249,139,448字节；本地临时105,045,762字节→0，远端隔离复现副本和上传均回收。旧版代码回读只在B73新写前验证：B73多位置写入后旧版只读代表位置，须优先前向修复或单独验证完整证据兼容性，不可自动回退后宣称信息完整。不得旧库覆盖新写，清报告专用快照仍待正式报告完成可读后删除。
- The report-cleanup recovery snapshot is `/opt/neckline/data/archive/report-cleanup-20260913/pre-cleanup.db.gz`; retain only until the first post-cleanup scheduled report completes and is readable, then remove this cleanup-only snapshot. After service restoration, never restore this whole DB over new writes; any recovery requires a reviewed, scoped restoration. B72/B73 release rollback sets remain separate.
