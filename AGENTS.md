...
- Production runs **K10-v1.4 / Neckline 3.1.0 Build 39 / Schema 7**, backend release set **v3.1.0-b39**, since 2026-09-08. K9 and the earlier K8 chains are retired from active production and this worktree; do not reintroduce their runtime code, tables, routes, settings, compatibility shims, or UI placeholders. The explicit offline V3 migration owns the legacy deletion boundary; Git history is the archive.
...
- **Current production state (2026-09-08):** The user authorized the September 8 official evening report as B39's first formal run. It started at 21:49, then failed at title triage (`response_json_invalid` / `title_incomplete`) before any body or Tavily calls. At 21:53 the operator guard closed the durable switch and stopped/disabled worker and evening/morning timers to prevent scheduled repetition. Preserve this task and completed title checkpoints; do not blindly replay it or recover the retired 2,472-article incident batch. See `PROJECT_PLAN.md` for the blocker and evidence.
