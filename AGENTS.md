...
- Production is **K10-v2 / Neckline 3.2.0 Build 61 / Schema 8**, backend release set **v3.2.0-b61**, since 2026-09-10. K9 and the earlier K8 chains are retired from active production and this worktree; do not reintroduce their runtime code, tables, routes, settings, compatibility shims, or UI placeholders. The explicit offline V3 migration owns the legacy deletion boundary; Git history is the archive.
...
- **Current production state (2026-09-10 B61 continuation):** B61 `99a83b3` is deployed in Ningbo, API active+enabled, run control open. At 21:49:41 the official recovery CLI resumed the same `task_c9c0feab5c83a08e8a17138ebfb0041a` / `scan_d39fbd5c631ea9a7df5f569dbd432b4d`, preserving all 44 paid title batches. Dedicated `neckline-trial-20260910-evening-b61.service` runs this task and then its successful report notification only. Normal worker/timers remain inactive+disabled. The user explicitly requires report completion, readable client output and actual push before this task ends. Preserve all old reports/choices/D1-D2 and do not resume unrelated historical work. BYOK is deepseek-flash. Current rollback set `/opt/neckline/data/backups/v3.2.0-b61-predeploy/` preserves B60 runtime and pre-continuation data; never overwrite subsequent writes. See PROJECT_PLAN.md / execution record section 26.
- **Current released baseline:** 3.2.0 / both platforms Build 61 (B58 SOP, B59 BYOK, B60 pool filtering and B61 output/status/push repairs), following the latest `/Users/linotsai/Lino/whynotme/K10.md` (K10-v2). The fixed 1,089-company universe, removal of 80/40 full-text caps, labeled-unverified recommendation path and daily company selection supersede conflicting v1.4 implementation rules. Existing v1.4 publication evidence remains historical, not a new-version behavior contract.
