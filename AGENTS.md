...
- Production is **K10-v2 / Neckline 3.2.1 clients Build 62 / backend Build 65 / Schema 8**, backend release set **v3.2.1-b65**, since 2026-09-13. K9 and the earlier K8 chains are retired from active production and this worktree; do not reintroduce their runtime code, tables, routes, settings, compatibility shims, or UI placeholders. The explicit offline V3 migration owns the legacy deletion boundary; Git history is the archive.
...
- **Current production state (2026-09-13):** 3.2.1 backend B65 `35de0fa` is now deployed in Ningbo; clients remain B62. At 19:51 CST the user explicitly authorized deleting September 10's report and all earlier reports to start with clean opportunity/focus/performance screens. Five old scans and their report/publication/card/window/selection/evaluation trees were deleted with verified backup and isolated rehearsal. This supersedes the former requirement to retain the old September 10 report. Never restore/replay these deleted reports or resend their notifications. Configuration, fixed 1,089-company snapshots, source progress, market facts, paid usage/checkpoint audit and sent-notification suppression records remain intact. API/worker were restored; all four timers remain enabled, run control open and notification readiness ready. Mac's actual three screens are empty after refresh; iOS reads the same cleared backend and needs an online refresh. See `archive/v3.2.1-b59_execution.md` section 17.
- **September 13 evening incident:** Scheduled task `task_00437ea0ac27c8601b8f5711494dc801` / scan `scan_0cfbef8493c21a2d0ed62bee9992caf5` started at 21:00:11 and failed at 21:03:15. Offline replay located `v2_profiles.retrieve_company_context` iterating a null optional `source_ref` at the 17th title batch. Preserve 16 paid completed batches (560,623 tokens); no blind rerun, no new task or input window. B65 fixes optional null/non-string source extraction without changing source fields or draft status; at 21:21 the same task resumed with all 16 paid checkpoints unchanged. No new task or budget change. Full source/profiles remain drafts with missing provenance, which must not be invented. See current Plan and section 17 evidence.
- **Current released baseline:** 3.2.1 / both clients Build 62 / backend Build 65. Research uses action-specific visible evidence, versioned local reads, shared source facts and interruption-safe query closure; recommendation roles and historical identity are separated. B71 price-reference DTO and prior recovery/fulltext fixes remain included. Follow `/Users/linotsai/Lino/whynotme/K10.md`: fixed 1,089-company universe, no 80/40 fulltext caps, labeled-unverified recommendation and daily company selection.
