...
- Production is **K10-v2 / Neckline 3.2.0 Build 62 / Schema 8**, backend release set **v3.2.0-b62**, since 2026-09-10. K9 and the earlier K8 chains are retired from active production and this worktree; do not reintroduce their runtime code, tables, routes, settings, compatibility shims, or UI placeholders. The explicit offline V3 migration owns the legacy deletion boundary; Git history is the archive.
...
- **Current production state (2026-09-10 B62 trial):** B62 `4500397` is deployed in Ningbo; API active+enabled and run control open. Same task `task_c9c0feab5c83a08e8a17138ebfb0041a` / scan `scan_d39fbd5c631ea9a7df5f569dbd432b4d` failed at 22:12:01 on title bookkeeping. All 44 paid title batches and private rejected replies are preserved; B63 local replay recovers the first complete reply without changing its 17 selected sources/reasons. B63 validation/release is in progress. Dedicated trial units are inactive; normal worker/timers stay inactive+disabled. User requires complete report, readable client output and actual push before ending; continue the same frozen task only. BYOK deepseek-flash. Rollback set `/opt/neckline/data/backups/v3.2.0-b62-predeploy/` preserves B61 runtime/current settings/data; never overwrite later writes. Mac remains B61 while locked; B62 packages are released. See PROJECT_PLAN.md / execution record sections 27–28.
- **Current released baseline:** 3.2.0 / both platforms Build 62 (B58 SOP, B59 BYOK, B60 pool filtering, B61 output/status/push and B62 diagnostic repairs), following the latest `/Users/linotsai/Lino/whynotme/K10.md` (K10-v2). The fixed 1,089-company universe, removal of 80/40 full-text caps, labeled-unverified recommendation path and daily company selection supersede conflicting v1.4 implementation rules. Existing v1.4 publication evidence remains historical, not a new-version behavior contract.
