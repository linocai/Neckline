...
- Production runs **K10-v1.4 / Neckline 3.1.0 Build 52 / Schema 7**, backend release set **v3.1.0-b52**, since 2026-09-09. K9 and the earlier K8 chains are retired from active production and this worktree; do not reintroduce their runtime code, tables, routes, settings, compatibility shims, or UI placeholders. The explicit offline V3 migration owns the legacy deletion boundary; Git history is the archive.
...
- **Current production state (2026-09-09 01:53 CST):** B52 is deployed; the same user-authorized September 8 evening task resumed with all 67 selected bodies and 13 supplemental bodies preserved. All 66 investigations are complete; final classification and publication are resuming after repairing the parallel aggregation container and paused-checkpoint recovery. Incomplete finalization must fail without publication. Resume closes a persisted assessment before new query planning; exhausted fulltext capacity blocks new requests, and denied empty results do not trigger repeated assessment. Research semantics are validated before completed caching; rejected historical derivatives remain auditable. Switch open and worker active, evening/morning timers await final report verification. Current task keeps execution revision 2 plus recorded runtimeRepair (32768 investigation output tokens; original start +21600 seconds); future schedules bind revision 3. Never recover the retired 2,472-article incident batch. See `PROJECT_PLAN.md`.
