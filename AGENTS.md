...
- Production is **K10-v2 / Neckline 3.3.0 release clients Build 69 / backend Build 71 / internal Schema 9 (public report Schema 8)**, backend release set **v3.3.0-b71**, since 2026-09-15. K9 and the earlier K8 chains are retired from active production and this worktree; do not reintroduce their runtime code, tables, routes, settings, compatibility shims, or UI placeholders. The explicit offline V3 migration owns the legacy deletion boundary; Git history is the archive.
...
- **Current production state (2026-09-15 21:31 CST):** 3.3.0 backend B71 `ddd8819` is deployed and published; clients remain Build69 (user confirmed iOS normal). Tonight's scheduled task `task_6b667feb769d323a1cf4ff2232135086` / scan `scan_059297733f849d3bb13409fa8c608d44` failed body validation at 21:11 and was resumed at 21:29 after B71. All 65 bodies now completed: two paid replies reused locally and only one necessary new body call. Original 152 completed checkpoints, paid ledger, frozen input, task identity and September 16 03:00:18 deadline are preserved. Research is ongoing, report and final push not yet complete. API, worker and four timers active+enabled; control remains open (`user_authorized_after_b70_live_smoke`). Heartbeat `neckline` checks every two minutes until same report is complete, actual API readable and both report pushes accepted; user authorizes necessary quickfix/release/same-task recovery. Do not touch Xcode. September 13 expired task and its ledger stay failed/unchanged; old deleted reports stay deleted. See `archive/v3.3.0-b69_execution.md` section 16.1.
...
- After the September 15 B71 release, retain only `v3.3.0-b70-predeploy` and `v3.3.0-b71-predeploy`, restoring B69/B70 code respectively on the current Schema9 without DB restoration. Verified gzip snapshots and runtime/evidence totaled 218,345,711 bytes (approximately 0.203 GiB) at cleanup; B71 pre/post share one inode. Superseded B69 recovery set, remote uploads/rehearsals and this fix's local temporary/fixture/build data were removed after ownership/open-handle/content checks. Private receipts prohibit automatic Schema downgrade; never overwrite new writes with old snapshots.
- The report-cleanup recovery snapshot is `/opt/neckline/data/archive/report-cleanup-20260913/pre-cleanup.db.gz`; retain only until the first post-cleanup scheduled report completes and is readable, then remove this cleanup-only snapshot. After service restoration, never restore this whole DB over new writes; any recovery requires a reviewed, scoped restoration. B70/B71 release rollback sets remain separate.
