...
- After the 2026-09-14 B69 release, retain only `v3.2.1-b68-predeploy` and `v3.3.0-b69-predeploy`, restoring B67/B68 code respectively. Verified gzip snapshots total approximately 0.293 GiB with runtime/evidence; identical B68 pre/post files share one inode, B69 pre/post retain Schema8/9 separately. Narrow 9→8 rollback requires stopped writers, closed control, no unresolved calls and zero private receipts; nonempty receipts prohibit automated downgrade and require a forward repair. Never overwrite new business writes with an old DB. Older recovery sets and this release's temporary uploads/rehearsals were removed after verification.
- The report-cleanup recovery snapshot is `/opt/neckline/data/archive/report-cleanup-20260913/pre-cleanup.db.gz`; retain only until the first post-cleanup scheduled report completes and is readable, then remove this cleanup-only snapshot. After service restoration, never restore this whole DB over new writes; any recovery requires a reviewed, scoped restoration. B68/B69 release rollback sets remain separate.
