...
- Production runs **K10-v1.4 / Neckline 3.1.0 Build 46 / Schema 7**, backend release set **v3.1.0-b46**, since 2026-09-08. K9 and the earlier K8 chains are retired from active production and this worktree; do not reintroduce their runtime code, tables, routes, settings, compatibility shims, or UI placeholders. The explicit offline V3 migration owns the legacy deletion boundary; Git history is the archive.
...
- **Current production state (2026-09-08):** The user authorized diagnosis, repair and completion of the September 8 official evening report after B39 title triage failed. B46 validates evidence before caching and revalidates invalid legacy results at controlled recovery. Explicit runtime repair gives investigation maxTokens 32768 and a 21600-second deadline from the unchanged original start; the same frozen task resumed at 23:35; switch open and worker active, timers pending this run's verification. Preserve completed checkpoints and original cutoff; never recover the retired 2,472-article incident batch. See `PROJECT_PLAN.md` for current progress and evidence.
