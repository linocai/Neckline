...
- **Current production state (2026-09-10 21:04 authorization):** B59 is deployed in Ningbo and API remains active+enabled. The user authorized opening durable run control and one real evening trial for the fixed 2026-09-10 21:00 cutoff. The exact trial task `task_c9c0feab5c83a08e8a17138ebfb0041a` failed at 21:09:52 in title screening because a model result referenced outside-pool company `300487.SZ`. Its transient runner has exited; run control remains open as requested, while normal worker and all timers stay inactive+disabled. No automatic/manual paid retry was launched. The failed structured checkpoint is retained privately; do not overwrite or blindly replay it. The user's currently selected connection is deepseek-flash. Do not recover old failed tasks or replay the retired 2,472-article incident batch. Preserve the September 8 report, original selections and D1/D2 windows, and the failed September 9 morning report. The release rollback set `/opt/neckline/data/backups/v3.2.0-b59-predeploy/` predates the trial; verify all subsequent writes before any restore. Trial backup is `/opt/neckline/data/backups/trial-b59-20260910-2100-pre/`. Bindings remain `k10-v2-production` rev 1 and `k10-v2-execution-production` rev 1. See PROJECT_PLAN.md and execution record section 24.
...
- **Provider authorization boundary:** The B59 release was validated offline only. The user subsequently authorized the single 2026-09-10 21:00 real evening trial, including that task's required provider calls; it has now finished unsuccessfully (22 calls / 801,379 tokens). Further paid recovery is a new action, not a continuation silently authorized by that completed one-trial request. Do not run separate balance/permission probes, unrelated live validation, old task recovery, or routine scheduling without scope support. Tests still require isolated databases and deterministic/network-denied transports. A single live trial does not establish stable provider success, selection performance or a daily token total. Fixed company profiles retain their original draft/provenance status.
