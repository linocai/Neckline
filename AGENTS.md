...
- Production is **K10-v2 / Neckline 3.2.0 Build 64 / Schema 8**, backend release set **v3.2.0-b64**, since 2026-09-10. K9 and the earlier K8 chains are retired from active production and this worktree; do not reintroduce their runtime code, tables, routes, settings, compatibility shims, or UI placeholders. The explicit offline V3 migration owns the legacy deletion boundary; Git history is the archive.
...
- **Current production state (2026-09-10 B64 continuation):** B64 `0c0a789` is deployed in Ningbo; API active+enabled, run control open. At 22:58:40 the official CLI resumed the original task `task_c9c0feab5c83a08e8a17138ebfb0041a` / scan `scan_d39fbd5c631ea9a7df5f569dbd432b4d`; all 216 completed checkpoints, including 44 paid title batches and 13 understood articles, are unchanged. Dedicated `neckline-trial-20260910-evening-b64.service` continues this task and its successful report notification only; normal worker/timers and old tasks remain paused. User requires the complete report, readable output and actual push before ending. BYOK deepseek-flash. Rollback set `/opt/neckline/data/backups/v3.2.0-b64-predeploy/` preserves B63 runtime/current settings/data; never overwrite later writes. Mac remains B61 while locked, B64 staging and B61 backup verified; iOS archive ready for Xcode. See PROJECT_PLAN.md / execution record section 29.
- **Current released baseline:** 3.2.0 / both platforms Build 64 (B58–B63 fixes plus B64 research evidence filtering and paid-response reuse), following the latest `/Users/linotsai/Lino/whynotme/K10.md` (K10-v2). The fixed 1,089-company universe, removal of 80/40 full-text caps, labeled-unverified recommendation path and daily company selection supersede conflicting v1.4 implementation rules. Existing v1.4 publication evidence remains historical, not a new-version behavior contract.
