...
@MainActor @Observable final class AppModel {
    ...
    func resetForConnectionChange() {
        advanceConnectionGeneration()
        publications = []; companyWindows = []; selectionDetails = []; scanSummaries = []; results = nil; configuration = nil; usage = nil; providers = []; tavilyKeySet = false
    ...
    func refresh() async {
        ...
        do {
            ...
            let newResults = try? await resultTask
            let newConfiguration = try? await configurationTask
            let newUsage = try? await usageTask
            guard isCurrent(generation) else { return }
