...
**3.2.0 / 双端 Build 64 / K10-v2 / Schema 8**，后端发布集合为 `v3.2.0-b64`。K9 已退出活动生产。
[下载安装包与校验值](https://github.com/linocai/Neckline/releases/tag/v3.2.0-b64)；Mac 当前安装 B61，下一次换装与画面验收等待解锁；iOS 通过 Xcode 由用户直接安装，本次不生成 IPA。

**2026-09-10 原晚报正在B64继续运行。** 22:58:40官方恢复同一任务，全部216个完成检查点和原账单保留；44批标题、13篇正文和已取证资料继续复用。B64局部处理不可用引用、命题改写和重复全文申请，缺证据仍明确标为待核。成功后只派送本报告；API active+enabled、开关open，常规worker/timer和旧任务暂停。
...
B64 通过 1,164 项离线回归、真实失败结果回放与同任务恢复测试，以及双端签名归档和严格验签；仅代码升级，部署前后 85 表数据未变。当前报告尚未完成，最终输出以任务状态为准；原 TuShare 采集缺口保留。
...
**Build 64 已发布双端 BYOK**：设置 → 模型配置，可保存多组 HTTPS Chat Completions 连接、修改端点和模型 ID、替换或清除 Key，启用一组即切换新任务的连接。API 基础地址自动补 `/chat/completions`；密钥留空保留，跨服务商地址更新须同时换 Key 或清除旧 Key。客户端和后端已一同发布；Mac 已换装，iOS 安装 Build 64 后即可使用。
...
发布源码 `0c0a789d7c3cde0b0020400dacfe1dde7787438a`，不可变标签 `v3.2.0-b64`；后续文档提交不移动标签。
...
本地签名归档：`/Users/linotsai/Lino/releases/Neckline/v3.2.0-b64-20260910/`；
后端包、wheel 与 runtime manifest：`/opt/neckline/releases/v3.2.0-b64/`。

本次只更新代码与 wheel；Schema 8、通知 Schema 2 和 85 张表全部数据保持不变，前后备份校验值相同。
服务器恢复集 `/opt/neckline/data/backups/v3.2.0-b64-predeploy/` 包含 B63 代码、环境／绑定、切换前后数据库和回执。
