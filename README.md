...
API 可读取；原单次报告已完成并推送，专用进程已退出。常规worker、早晚报和行情更新/重试定时器已启用，通知就绪；旧待执行分析已隔离，不重放已完成报告。
...
后端部署保持Schema 8、通知Schema 2和85表数据不变。随后正式启用只隔离一条旧任务并更新运行开关；常规行情/评价已开始追加新记录，不能再拿部署前快照覆盖现役库。
