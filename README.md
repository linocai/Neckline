...
**生产扫描仍停用。** K10 worker 和晚/晨定时器保持停止并禁用，API 保留读取。用户已弃用的 2,472 篇事故批次及关联任务已备份后定点取消，不会补跑。

本地目标为 **3.0.6 / 双端 Build 38 / Schema 6**，尚未发布。B37 的总预算与资料包初筛方案已被替换：全部标题先由 DeepSeek 理解，经全局去重排序及只减不补的标题终检后，晚间最多深读 80 篇、晨间最多 40 篇。
824 项后端回归及双端三条构建通过。隔离实测完成1,430条标题筛选和37篇正文深读，但公司比较校验失败使结果为 partial、未生成正式候选；全链路验收尚未通过，生产继续停用。详情见 PROJECT_PLAN。
...
Tavily 只核验入选事件，默认提供搜索摘录；任何新增全文也受同轮文章限额约束。
来源事实未变可复用，正文版本、规则或模型输入改变则失效；时效、价格和两日判断仍重新核验。
不设整轮或整日金额、token、模型调用、搜索调用总上限。逐次实际用量账、有限重试、缓存与持久暂停保留；未知调用结果单列，不伪装成功或可免费重试。

Schema 6 支持生产 4→6 与本地 5→6，删除未发布的预算/模板表，迁移默认持久暂停。设置页显示标题理解、同事项合并、入选、深读完成/缺正文、搜索摘录、请求回执与缓存。
...
本轮先用 2,472 条标题规模完成离线协议验收，再用最新一个有效时窗做隔离真实验证；合成模型结果不作为真实筛选质量证据。
