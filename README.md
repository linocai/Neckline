...
**3.2.1 / 双端 Build 62 / 后端 Build 64 / K10-v2 / Schema 8**，后端发布集合为 `v3.2.1-b64`。K9 已退出活动生产。
...
**9月10日21:00原晚报已完成并推送。** 9月11日01:06:26发布7家公司、22个事件全部执行完成，01:06:28两台设备APNs均返回200；B71修复最终读取错误，实际API及当前Swift解码通过。原截止与已付结果不变，原采集缺口保留；当天完成期限已错过。API/worker active+enabled、开关open，早晚报和行情更新/重试定时器已启用；旧未执行分析已隔离。
...
后端B63–B64已按用户授权启用正式生产：每个交易日09:00晨报、交易日前一自然日21:00晚报，按交易所日历处理周末和节假日。136项调度/链路回归及10项恢复/版本检查通过；异地自动备份功能已删除，发布回退副本保留。[后端发布与校验值](https://github.com/linocai/Neckline/releases/tag/v3.2.1-b64)。
...
后端发布源码 `35de0fa82333e38a4f45cd89c3659a2a70258d47`，不可变标签 `v3.2.1-b65`；后续文档提交不移动标签。
服务器 `ser657204219523`（`114.66.2.205`），数据库 `/opt/neckline/data/neckline.db`，公网 `https://nk.linotsai.top`。
API可读取；9月10日及此前所有旧报告、关注和成绩已于9月13日19:51按用户授权清空。常规worker、早晚报和行情更新/重试定时器保持启用，通知就绪；不得恢复、重放或重推已删除报告。配置、股票池、资讯进度、行情与付费/送达账本保留。
9月13日21:00晚报已按时触发，但21:03因本地公司资料的空来源字段处理缺陷失败；16/30标题批次及560,623 token账本已保留，Build65已修复并于21:21恢复同一任务，完整报告仍以实际完成结果为准，禁止新建重复报告。
...
Mac `/Applications/Neckline.app` 当前为 3.2.1（62），Developer ID 严格验签、通用架构、单实例启动通过；清理后实际机会、关注、选股表现三个页面均为空。iOS联网刷新后读取相同空态。
...
后端包、wheel 与 runtime manifest：`/opt/neckline/releases/v3.2.1-b65/`。

后端部署保持Schema 8、通知Schema 2和85表数据不变。随后正式启用隔离一条旧任务并更新运行开关；19:51另按用户新授权清除旧报告及关联记录。不得用部署前或清理前快照覆盖现役库。
服务器恢复集 `/opt/neckline/data/backups/v3.2.1-b65-predeploy/` 包含 B64 代码、环境/绑定、全部当前数据的前后备份和回执；禁止覆盖后续写入。
2026-09-13 已按用户授权清理历史备份：当前仅保留 B64、B65 两套 predeploy 恢复集（分别可退回 B63、B64 代码），相同前后快照已去重，合计约 0.60 GiB；更早历史记录里的恢复路径已不再存在。正式数据库、行情、发布包及下述 Mac 副本保留。
