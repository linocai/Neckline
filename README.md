...
**当前发布为3.5.1（86）**：K10-v2，后端已上线；Mac已替换，锁屏导致启动验收待完成。iOS签名归档就绪，由用户Xcode安装，无IPA。[发布下载与校验值](https://github.com/linocai/Neckline/releases/tag/v3.5.1-b86)，源码`6c3ab502549e43123be3db0dcea724ebfb1a3ae9`。

B86修复付费研究跨片尾不能保存、共享事实改变续跑输入及固定池过滤丢失；保留B83/B85既有修复。23:13原任务继续，六项原付费回复本地重验零外呼。正式报告及完成APNs仍待验收，当前正在核查本地恢复耗时。策略、模型、冻结配置和四正式timer不变。
...
最终89项相关回归通过，双端OS27签名归档、模拟器build-for-testing、新旧同绑定API演练通过。B86本地临时742,077,910、远端临时12,184,713,101字节已核验回收；恢复集压缩并校验，Mac启动与隐藏旧包去重待解锁。见[本版第13节](archive/v3.5.1-b82_execution.md#13-今晚第四轮快修-b86进行中)。
