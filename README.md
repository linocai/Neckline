...
**3.3.0 / 双端发布 Build 69 / 后端 Build 74 / K10-v2 / 内部 Schema 9**，公开报告协议保持 Schema 8，后端发布集合为 `v3.3.0-b74`。K9 已退出活动生产。
[后端B74与校验值](https://github.com/linocai/Neckline/releases/tag/v3.3.0-b74)；[客户端B69下载](https://github.com/linocai/Neckline/releases/tag/v3.3.0-b69)；Mac 已安装并验收 3.3.0（69）；iOS 签名归档就绪，通过 Xcode 由用户直接安装，不生成 IPA。
...
后端发布源码 `ee5c2dee13d03857cfcc85bb7b88d99215b02a16`，不可变标签 `v3.3.0-b74`；后续文档提交不移动标签。服务器 `ser657204219523`（`114.66.2.205`），数据库 `/opt/neckline/data/neckline.db`，公网 `https://nk.linotsai.top`。

2026-09-16 00:10 CST核验：B74 `be15db7`已部署并发布GitHub，客户端仍B69；9月16日00:10恢复9月15日晚报原task，968完成检查点、931外呼审计、冻结输入和03:00:18截止保留。明确HTTP400输入获得一次固定检查点续做，尚待实际执行结果；旧失败审计不得误当新增故障，维护期间无新增失败推送。报告/最终推送尚未完成，心跳每2分钟继续，Xcode暂停。
...
B74相对B69没有App源码改动，沿用B69客户端与既有验签／验收记录；本次后端快修没有新增Apple构建或换装。
本地后端发布目录：`/Users/linotsai/Lino/releases/Neckline/v3.3.0-b74-20260915/`；服务器正式后端包、wheel与manifest：`/opt/neckline/releases/v3.3.0-b74/`；GitHub五份后端资产与本地SHA256一致。B69客户端归档仍在原`v3.3.0-b69-20260914/`目录。
...
Mac可恢复副本为`/Users/linotsai/Lino/app_backups/Neckline-v3.2.1-build62-pre-v3.3.0-b69-20260914.app`。根目录`root:root /0755`、数据库`neckline:neckline /0600`已经复核。B74临时构建、私有原回复、用户／系统测试隔离目录和远端上传／演练产物已按精确归属核验清理，签名归档、安装包、必要证据与上述恢复集保留。
