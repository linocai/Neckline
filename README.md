...
**3.2.1 / 双端 Build 62 / 后端 Build 68 / K10-v2 / Schema 8**，后端发布集合为 `v3.2.1-b68`。K9 已退出活动生产。
...
**9月13日原晚报尚未完成。** 已从保存点推进到68/86事件完成，随后被Tavily套餐额度拒绝；B68修复识别和受控续做，等待用户恢复额度。付费执行开关临时关闭，API与定时安排保留。9月10日及更早报告已按用户授权清除，不重跑或恢复。
...
3.2.1（62）修复全部提前撤回的窗口过 D2 后，旧撤回/风险仍占首页的问题；当前窗口提醒保留，历史默认折叠。独立复查、5 项 XCTest、版本治理、三条双端构建、严格验签及双端实际 API 界面验收通过。保留 B61 的回读协议、冗余修复及小样本真实链路结果；该次发布无付费调用，原报告、选择与推送未改写。随后9月13日19:51按用户新授权清空全部旧报告及关注/成绩，现等待新正式报告。

后端B63–B64已按用户授权启用正式生产：每个交易日09:00晨报、交易日前一自然日21:00晚报，按交易所日历处理周末和节假日。136项调度/链路回归及10项恢复/版本检查通过；异地自动备份功能已删除，发布回退副本保留。[当前后端发布与校验值](https://github.com/linocai/Neckline/releases/tag/v3.2.1-b68)。
...
**B59起支持双端 BYOK**：设置 → 模型配置，可保存多组 HTTPS Chat Completions 连接、修改端点和模型 ID、替换或清除 Key，启用一组即切换新任务的连接。API 基础地址自动补 `/chat/completions`；密钥留空保留，跨服务商地址更新须同时换 Key 或清除旧 Key。当前后端与 Mac 均为 3.2.1（61），iOS 通过 Xcode 安装。
...
后端发布源码 `20cdbf6ba197bca0e57ab4eaf1fda8fff7ddbc52`，不可变标签 `v3.2.1-b68`；后续文档提交不移动标签。
服务器 `ser657204219523`（`114.66.2.205`），数据库 `/opt/neckline/data/neckline.db`，公网 `https://nk.linotsai.top`。
API可读取；9月10日及此前所有旧报告、关注和成绩已于9月13日19:51按用户授权清空。早晚报和行情更新/重试定时器保持启用、通知就绪；9月14日00:03因Tavily额度拒绝，付费执行开关暂closed，worker已安全停止但保持enabled；不得恢复、重放或重推已删除报告。配置、股票池、资讯进度、行情与付费/送达账本保留。
9月13日原晚报在保存点续跑后，68/86事件完成、3项待核实结论、15项因Tavily套餐额度失败；23:49终止，未发布报告。B68已修复432/433识别和受控续做授权，等待用户恢复Tavily额度后开放开关并恢复原task。冻结输入、23,633,932 token账本及9月14日03:00:13原时限保留，不重跑、不默认延长时限。准确恢复入口与证据见当前Plan链接的版本记录第17节。
...
Mac `/Applications/Neckline.app` 当前为 3.2.1（62），Developer ID 严格验签、通用架构、单实例启动通过；旧报告清理后未恢复；本轮Mac实际刷新显示9月13日原任务失败、尚未发布，iOS读取同一后端。
...
后端包、wheel 与 runtime manifest：`/opt/neckline/releases/v3.2.1-b68/`。

后端部署保持Schema 8、通知Schema 2和85表数据不变。随后正式启用隔离一条旧任务并更新运行开关；19:51另按用户新授权清除旧报告及关联记录。不得用部署前或清理前快照覆盖现役库。
服务器恢复集 `/opt/neckline/data/backups/v3.2.1-b68-predeploy/` 包含 B67 代码、环境/绑定、全部当前数据的前后备份和回执；禁止覆盖后续写入。
2026-09-13 已按用户授权清理历史备份：当前仅保留 B67、B68 两套 predeploy 恢复集（分别可退回 B66、B67 代码），相同前后快照已去重，合计约 0.83 GiB；更早历史记录里的恢复路径已不再存在。正式数据库、行情、发布包及下述 Mac 副本保留。
清理专用恢复点 `/opt/neckline/data/archive/report-cleanup-20260913/pre-cleanup.db.gz` 已验可恢复；首份新正式报告完成且可读后回收，禁止整库覆盖后续写入。
...
保持根目录 `root:root /0755`、数据库 `neckline:neckline /0600`，恢复前暂停写入，恢复后核对完整性、健康、鉴权、实际配置和本次获准的调度状态。

用户已清除全部旧报告及其选择/窗口/成绩；付费和送达审计保留，不能为了新版本重放旧报告。
此前 B39–B53 故障、恢复及旧执行参数只作历史证据，见 [执行记录](archive/v3.1.0-b39_execution.md)；
本次发布与恢复集的详细证据见 [3.2.1 执行记录第 17 节](archive/v3.2.1-b59_execution.md)。数据库和凭据只留服务器，不公开。
