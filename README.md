...
**3.3.0 / 双端发布 Build 69 / 后端 Build 71 / K10-v2 / 内部 Schema 9**，公开报告协议保持 Schema 8，后端发布集合为 `v3.3.0-b71`。K9 已退出活动生产。
[后端B71与校验值](https://github.com/linocai/Neckline/releases/tag/v3.3.0-b71)；[客户端B69下载](https://github.com/linocai/Neckline/releases/tag/v3.3.0-b69)；Mac 已安装并验收 3.3.0（69）；iOS 签名归档就绪，通过 Xcode 由用户直接安装，不生成 IPA。
...
**3.3.0后端Build71已发布并部署。** B69排除日报招股书补证，限制查证于当前事件必要问题；正文按结构定位，公司资料按动作读取，共享来源事实随新增内容更新，原始付费回复可本地重验。B70修复必要限定遗漏、普通词内误报、跨句共享及旧目录恢复；B71从事件唯一来源补齐正文事实省略的重复引用，原付费回复本地重验通过即可复用。缺事实、来源冲突和无效JSON仍拒绝。实际请求容量检查不新增策略总用量上限；字符回放和离线测试不能证明真实费用节省。
...
后端发布源码 `ddd8819697ea9225234e4fb2665efd0e79133f3a`，不可变标签 `v3.3.0-b71`；后续文档提交不移动标签。服务器 `ser657204219523`（`114.66.2.205`），数据库 `/opt/neckline/data/neckline.db`，公网 `https://nk.linotsai.top`。

2026-09-15 21:31 CST核验：B71 API、worker、四timer active+enabled，control open，四配置与通知ready。今晚21:00:18原任务在21:11正文校验失败，B71发布后21:29受控恢复；65篇正文已完成，2篇复用旧付费回复、1篇必要新调用，正在事件研究。原输入、已完成检查点、账本与9月16日03:00:18截止不变；监控每2分钟跟踪至报告完成、实际可读及双端推送。报告最终状态尚待核验。
...
B71相对B69没有App源码改动，沿用B69客户端与既有验签／验收记录；本次后端快修没有新增Apple构建或换装。
本地后端发布目录：`/Users/linotsai/Lino/releases/Neckline/v3.3.0-b71-20260915/`；服务器正式后端包、wheel与manifest：`/opt/neckline/releases/v3.3.0-b71/`；GitHub五份后端资产与本地SHA256一致。B69客户端归档仍在原`v3.3.0-b69-20260914/`目录。

服务器仅留`/opt/neckline/data/backups/v3.3.0-b70-predeploy/`和`v3.3.0-b71-predeploy/`，分别保留B69／B70代码恢复能力，清理时合计218,345,711字节（约0.203GiB）。gzip解压哈希已验证，B71相同pre/post共用inode；部署86表指纹未变，B71与B70均已在当前Schema9隔离副本验证，可仅回退代码并保留新写。有私有回执时禁止自动Schema降级；旧快照不能覆盖后续业务写入。
...
Mac可恢复副本为`/Users/linotsai/Lino/app_backups/Neckline-v3.2.1-build62-pre-v3.3.0-b69-20260914.app`。根目录`root:root /0755`、数据库`neckline:neckline /0600`已经复核。B71临时构建、私有原回复、用户／系统测试隔离目录和远端上传／演练产物已按精确归属核验清理，签名归档、安装包、必要证据与上述恢复集保留。
