...
**当前发布为3.5.1（87）**：K10-v2，后端已上线；Mac已替换，锁屏导致启动验收待完成。iOS签名归档就绪，由用户Xcode安装，无IPA。[发布下载与校验值](https://github.com/linocai/Neckline/releases/tag/v3.5.1-b87)，源码`a29559dd4d5a2d4dfef7e2dff4080cb1b099100e`。

B87修复资料检索反复编译正则造成的恢复耗时，固定池匹配结果不变；保留B83–B86付费回执、跨片持久化和共享输入恢复修复。原任务23:29继续，23:31已出现新研究调用/回复和补查结果。正式报告及完成APNs仍待验收；策略、模型、冻结配置和四正式timer不变。
...
B87相关19项回归通过，实际50,526次关键词判断旧/新一致、同一研究恢复结果相同；双端OS27签名归档、模拟器build-for-testing、新旧同绑定API演练通过。本轮临时物已精确回收（本地613,631,354、远端1,438,067,624字节），恢复集压缩校验；Mac启动与隐藏旧包去重待解锁。见[本版第14节](archive/v3.5.1-b82_execution.md#14-今晚第五轮快修-b87进行中)。
