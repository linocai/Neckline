...
后端发布源码 `ca5511c11db8be32762f2bd0d490f0868e1d221d`，不可变标签 `v3.2.1-b67`；后续文档提交不移动标签。
...
9月13日原晚报已在Build67下继续：57篇正文已全部通过，原文回读分流阻塞已修复。23:29续跑保留940条完成检查点及906条付费尝试记录，冻结输入和原恢复授权不变。完整报告及成功通知仍待终核，禁止新建重复报告。
...
Mac `/Applications/Neckline.app` 当前为 3.2.1（62），Developer ID 严格验签、通用架构、单实例启动通过；旧报告清理后未恢复；本轮Mac实际刷新显示9月13日原任务继续执行，iOS读取同一后端。
...
后端包、wheel 与 runtime manifest：`/opt/neckline/releases/v3.2.1-b67/`。

后端部署保持Schema 8、通知Schema 2和85表数据不变。随后正式启用隔离一条旧任务并更新运行开关；19:51另按用户新授权清除旧报告及关联记录。不得用部署前或清理前快照覆盖现役库。
服务器恢复集 `/opt/neckline/data/backups/v3.2.1-b67-predeploy/` 包含 B66 代码、环境/绑定、全部当前数据的前后备份和回执；禁止覆盖后续写入。
2026-09-13 已按用户授权清理历史备份：当前仅保留 B66、B67 两套 predeploy 恢复集（分别可退回 B65、B66 代码），相同前后快照已去重，合计约 0.70 GiB；更早历史记录里的恢复路径已不再存在。正式数据库、行情、发布包及下述 Mac 副本保留。
