...
后端发布源码 `62990f07bdd829bf76447eb776c1b4725b72b125`，不可变标签 `v3.2.1-b66`；后续文档提交不移动标签。
...
9月13日原晚报已在Build66下恢复，57篇正文全部通过：复用50篇完成结果及6份已付回复，仅1篇局部纠正新增16,336 token。当前继续后续研究，完整报告仍以实际完成结果为准，禁止新建重复报告。
...
后端包、wheel 与 runtime manifest：`/opt/neckline/releases/v3.2.1-b66/`。

后端部署保持Schema 8、通知Schema 2和85表数据不变。随后正式启用隔离一条旧任务并更新运行开关；19:51另按用户新授权清除旧报告及关联记录。不得用部署前或清理前快照覆盖现役库。
服务器恢复集 `/opt/neckline/data/backups/v3.2.1-b66-predeploy/` 包含 B65 代码、环境/绑定、全部当前数据的前后备份和回执；禁止覆盖后续写入。
2026-09-13 已按用户授权清理历史备份：当前仅保留 B65、B66 两套 predeploy 恢复集（分别可退回 B64、B65 代码），相同前后快照已去重，合计约 0.62 GiB；更早历史记录里的恢复路径已不再存在。正式数据库、行情、发布包及下述 Mac 副本保留。
