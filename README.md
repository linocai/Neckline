...
**当前发布为3.5.0（81）**：K10-v2，后端及macOS已部署，iOS签名构建就绪，通过Xcode由用户安装，不生成IPA。[发布下载与校验值](https://github.com/linocai/Neckline/releases/tag/v3.5.0-b81)；源码`626c902097a91481513ac9792257309858c9e031`。

新晨报交易日 **08:30** 开始，**09:20** 前提供可读结果；晚报保持前一自然日 **21:00** 开始，不设整报业务截止。局部失败可交付部分报告；排序失败或请求费用待确认时可查看安全材料或诊断，不产生正式推荐、选择或观察样本。**四个定时器和Codex心跳仍关闭，此次发布不恢复旧任务。**

内部 **Schema10 / 新公开报告Schema9**，历史Schema8报告保持可读。生产迁移与旧数据保留、配置读取、健康检查通过；部署时35个任务均终态、无未决外呼，历史晨/晚报告仍失败且未发布。生产固定1,089公司池和完整资料保持显式绑定，资料为`local_draft_awaiting_user`，不冒充已核实。

Neckline包含SwiftUI macOS/iOS客户端与FastAPI后端。双端签名归档及定向前端回归通过；iOS完整逐屏交互验收尚未完成，iPhone18Pro开发符号待就绪。真实模型质量、耗时、费用与首次正式报告送达未由本次离线测试证明。详细验证边界、迁移恢复与资源证据见[本版记录](archive/v3.5.0-b78_execution.md)。
...
初始化／升级顺序：确认目标和恢复路径 → 受控schema迁移 → 导入固定资料 → 登记配置修订 → 绑定策略快照 → 核对API配置响应。3.5.0内部Schema10新增研究轮次、晨报工作项、搜索回执和交付材料/元数据，公开新报告Schema9。生产副本9→10→9→10演练通过；正式迁移保留原业务行与付费账本。API启动和GET不执行迁移。回退B75只允许在停机核验五张新表为空、旧数据未变后受控降级；已有新写时停止并前向修复，禁止旧库覆盖新写。
...
发布源码`626c902097a91481513ac9792257309858c9e031`，不可变标签`v3.5.0-b81`；后续文档提交不移动标签。服务器`ser657204219523`（`114.66.2.205`），数据库`/opt/neckline/data/neckline.db`，公网`https://nk.linotsai.top`。2026-09-21 15:49 CST核验125份runtime文件与发布manifest一致，数据库完整性正常，API/worker active，四timer inactive/disabled且无下一触发时间。
