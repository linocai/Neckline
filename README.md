...
**3.1.0 / 双端 Build 43 / K10-v1.4 / Schema 7**，后端发布集合为 `v3.1.0-b43`。K9 已退出活动生产。
[下载安装包与校验值](https://github.com/linocai/Neckline/releases/tag/v3.1.0-b43)；Mac 已换装并启动，iOS 通过 Xcode 由用户直接安装，本次不生成 IPA。
...
**Build 43 已修复并续跑原正式任务。** 9 月 8 日晚报曾因标题输出的数组容器不兼容而失败；22:55 再续原冻结任务，保留全部 17 个成功标题批次；同时修复自动分段丢失恢复状态。开关 open、worker active，既有晚晨排程待本轮验收后恢复。
...
源码提交 `d2dd3d8a443770766299d01c310f1ce7ed5ea912`，不可变标签 `v3.1.0-b43`；后续发布记录提交不移动标签。
...
B43 于 22:55 恢复数据库 run control 为 `open`，worker active+enabled；晚／晨 timer 待本轮验收恢复，行情更新／重试 timer 保持原态。标题 policy `k10-title-triage-v1@1` 已按既有批准配置原样登记；首次启动暴露的 readiness／部署登记缺口见 PROJECT_PLAN。

Mac 位于 `/Applications/Neckline.app`，Developer ID 严格验签、通用架构和单实例启动通过；安装包确认 Build 43；四范围配置和实际 API 状态通过服务器验收。
Mac 尚未公证，网络下载后可能被 Gatekeeper 拦截；严格签名通过不代表公证通过。
iOS 真机签名归档及工程版本／签名配置已就绪，由用户通过 Xcode 安装，不导出 IPA。
本地签名归档与包位于 `/Users/linotsai/Lino/releases/Neckline/v3.1.0-b43-20260908/`。
...
Mac 最近可恢复备份为 `/Users/linotsai/Lino/app_backups/Neckline-v3.1.0-build40-pre-b41-20260908.app`。B43 服务器完整备份位于 `/opt/neckline/data/backups/v3.1.0-b43-predeploy/`；代码部署前后数据库哈希一致。
