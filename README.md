...
后端发布源码 `fa94f81e8e0bd403c4ab0241191af564d0772b68`，不可变标签 `v3.2.1-b64`；后续文档提交不移动标签。
...
9月13日21:00晚报已按时触发，但21:03因本地公司资料的空来源字段处理缺陷失败；16/30标题批次及560,623 token账本已保留，等待修复后恢复同一任务，禁止新建重复报告。
...
后端包、wheel 与 runtime manifest：`/opt/neckline/releases/v3.2.1-b64/`。

后端部署保持Schema 8、通知Schema 2和85表数据不变。随后正式启用隔离一条旧任务并更新运行开关；19:51另按用户新授权清除旧报告及关联记录。不得用部署前或清理前快照覆盖现役库。
服务器恢复集 `/opt/neckline/data/backups/v3.2.1-b64-predeploy/` 包含 B63 代码、环境/绑定、全部当前数据的前后备份和回执；禁止覆盖后续写入。
2026-09-13 已按用户授权清理历史备份：当前仅保留 B63、B64 两套 predeploy 恢复集（分别可退回 B62、B63 代码），相同前后快照已去重，合计约 0.58 GiB；更早历史记录里的恢复路径已不再存在。正式数据库、行情、发布包及下述 Mac 副本保留。
