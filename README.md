...
**2026-09-10 晚间单次试跑失败，已停止执行。** 用户于 21:04 授权真实试跑，21:07 已打开持久运行开关并开始处理固定 21:00 截止的新任务。任务 21:09:52 停在标题筛选，原因是模型返回池外代码 `300487.SZ`；未进入正文研究、未发布。API active+enabled，单次执行进程已退出；常规 worker、早晚报告、行情和备份 timer 仍 inactive+disabled。9 月 8 日晚报两份样本、原用户选择及晨报失败检查点保留，不重放报告，不恢复已退役的 2,472 篇事故批次。
...
本轮通过 1,135 项离线后端回归、双端构建与签名归档、真实副本及生产只读 API 与 Swift 解码验收；代码升级前后全部数据保持一致，无迁移；未进行真实模型／搜索／行情／推送调用或余额／权限探测。上述为发布时边界；用户随后已授权本次单任务真实试跑，已于标题阶段失败。22 次模型调用合计 801,379 tokens，另有 TuShare 采集缺口；没有追加付费重试。
