...
✅ **已修(2026-08-04 A 组 A9-④,commit `4c00976`)**:六处全部拉平成「取不到就退诚实空态」
(`(try? c.decode(...)) ?? 默认`,同时覆盖**缺键**与**形状变了**两种情形 —— 后者正是本条
担心的"服务端日后动 `sectors` 形状"）。⚠ 一处**刻意不取"看起来自然"的默认值**:
`degraded` 缺键取 **`true`** 而不是 `false` —— 这个位说的是"这份报告完不完整",缺了它就是
**不知道**,取 `false` 等于替服务端保证"一切正常"(拿"没看"当"没有");`reason` 在这种情形下
补一句「服务端响应缺 degraded 字段,无法确认这份报告是否完整」,不给一句空白。
Swift 两例:①极端形状(整份报告只剩 `basketDaily` 一段)仍解得出且六个字段各自退到空态;
②`sectors` 从数组变成对象时**只降级那一个字段**,其余字段一个不丢。
...
✅ **已裁定 + 已实现(planner 裁定见 `199521f` / PROJECT_PLAN ⑭-B;实现 commit
`4c00976`,2026-08-04 A 组同批)**:新 reason **`card_corrupt` = 500**(⛔ **不进 404
家族** —— 404 是在说谎〔卡存在、只是读不出〕,且会与 `card_not_ready` 撞成同一类,
而两者要求的反应完全相反;决定性理由是卡为冻结件、`INSERT OR IGNORE` 永不覆盖 →
**坏了就是永久坏的**,当成"还没生成"处理 = 客户端永远重试、界面永远显示"卡还没生成"
= 静默永久失败)。落地要点:
- **检测点唯一在 store**(`basket_store._decode_card_json`),打 **ERROR** 不是 WARNING;
  ⛔ 不重建、不跳过坏字段(读侧糊过去 = 藏真数据)。
- 判据 = json 解不出 / 顶层不是对象 / **顶层一个内容键都没有**。⚠ 最后这条**刻意是
  "三者有其一"而不是"三者都要有"**:各消费方要的不是同一批键(⑧ 只读两份 spec、
  ⑩ 只读 `members`),"都得有"会把合法的**局部卡**判成损坏 —— 这不是推演,是施工中先按
  "都得有"写、全量套件 16 红反证出来的;误判代价还不对称(判错成损坏 = 用户看不到
  一张好卡且不可自愈)。
- 列表/详情端点(卡是内嵌可选字段)照返 **200** 但 reason 如实是 `card_corrupt`;
  报告 markdown 写「本篮卡数据损坏,已记录待排查」+ 明说等不来新的。
- 客户端 `APIError.cardCorrupt` +「这张卡的数据损坏了,需要排查」,`send()` 的
  **500 分支接进 `mapReason`**(未知 500 仍 fallback `.server`),⛔ 不进静默重试。
- **未覆盖的一处(如实登记,不在裁定文范围内)**:`positions_entry` 的开仓计划继承在
  卡损坏时仍报 `card_not_ready`(那条 reason 走的是 `plan.reason` 字段、另一套客户端
