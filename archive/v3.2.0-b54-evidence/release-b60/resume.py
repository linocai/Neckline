...
backup = db.parent / 'backups/v3.2.0-b60-predeploy'
...
with urllib.request.urlopen('http://127.0.0.1:8002/api/v1/health') as response:
    assert json.load(response)['releaseSet'] == 'v3.2.0-b60'
...
def snapshot():
    ...
    return {'paidTitleCount': len(titles), 'paidTitleSha256': digest(titles), 'oldTasksSha256': digest(old_tasks),
        'externalAttemptsSha256': digest(attempts)}

before = snapshot()
assert before['paidTitleCount'] == 22
...
value = {'taskId':task_id, 'scanId':scan_id, 'releaseSet':'v3.2.0-b60', 'frozenInputSha256':frozen,
