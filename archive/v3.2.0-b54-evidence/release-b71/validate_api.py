"""B71 read-only release acceptance; offline mode denies all socket connections."""
import argparse
import json
import os
...
if args.mode == 'offline':
    ...
    os.environ['API_TOKEN'] = 'offline-b71-release-token'
...
def verify(get):
    ...
    assert health['releaseSet'] == 'v3.2.0-b71' and health['status'] == 'ok'
    out['health'] = health
    path = Path(args.output)
    path.write_text(json.dumps(out, ensure_ascii=False, indent=2) + '\n')
    path.chmod(0o600)
    print(json.dumps({'mode': args.mode, 'configuredScopes': 4, 'bindings': 'K10-v2 rev1 / execution rev1',
        'runControl': 'ready', 'evening': 'published_partial', 'morning': 'empty', 'providerUnchanged': True,
