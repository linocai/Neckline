"""B66 read-only release acceptance; offline mode denies all socket connections."""
import argparse
import json
import os
...
if args.mode == 'offline':
    ...
    os.environ['API_TOKEN'] = 'offline-b66-release-token'
...
def verify(get):
    ...
    for window in ('evening', 'morning'):
        ...
        if window == 'evening':
            assert report.report.status == 'failed' and report.reason.reason == 'execution_paused'
    ...
    assert health['releaseSet'] == 'v3.2.0-b66' and health['status'] == 'ok'
    out['health'] = health
