"""B70 read-only release acceptance; offline mode denies all socket connections."""
import argparse
import json
import os
...
if args.mode == 'offline':
    ...
    os.environ['API_TOKEN'] = 'offline-b70-release-token'
...
def verify(get):
    ...
    for window in ('evening', 'morning'):
        ...
        if window == 'evening':
            assert report.report.status == 'failed' and report.reason.reason == 'research_execution_failed'
    ...
    assert health['releaseSet'] == 'v3.2.0-b70' and health['status'] == 'ok'
    out['health'] = health
...
if args.mode == 'offline':
    import sqlite3
    from neckline.k10.opportunity_discovery import validate_event_comparison
    from neckline.k10.research_store import read_research_state
    research = read_research_state(snapshot_id='research_ff9ee24afc47c5413af4f0495057864d', db_path=Path(args.db))
    assessments = research['assessments']
    assert len(assessments) == 13
    expected = {'301631.SZ': ('primary', 1), '301282.SZ': ('primary', 2),
        '301487.SZ': ('alternative', 3), '301360.SZ': ('alternative', 4), '300890.SZ': ('tied', 4)}
    assert {r['companyCode']: (r['role'], r['rank']) for r in assessments if r['rank'] is not None} == expected
    comparison = next(s['result']['conclusion'] for s in reversed(research['stageResults'])
        if s['result'].get('companyAssessments'))
    validate_event_comparison(summary=comparison['summary'], company_codes=[r['companyCode'] for r in assessments],
        comparisons={r['companyCode']: {'summary': r['summary'], 'rank': r['rank'], 'differences': {
            k: r[k] for k in ('role', 'priorityReason', 'gap', 'rankChangeConditions', 'twoDayReason', 'evidenceDisclosure')}}
            for r in assessments}, require_evidence_disclosure=True)
