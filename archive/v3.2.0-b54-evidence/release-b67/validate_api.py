"""B67 read-only release acceptance; offline mode denies all socket connections."""
import argparse
import json
import os
...
if args.mode == 'offline':
    ...
    os.environ['API_TOKEN'] = 'offline-b67-release-token'
...
def verify(get):
    ...
    for window in ('evening', 'morning'):
        ...
        if window == 'evening':
            assert report.report.status in {'completed', 'partial'} and report.report.availableAt
            assert report.report.reportId == 'report_scan_d39fbd5c631ea9a7df5f569dbd432b4d'
            assert not report.report.incompleteReviews
    ...
    assert health['releaseSet'] == 'v3.2.0-b67' and health['status'] == 'ok'
    out['health'] = health
    path = Path(args.output)
    path.write_text(json.dumps(out, ensure_ascii=False, indent=2) + '\n')
    path.chmod(0o600)
    print(json.dumps({'mode': args.mode, 'configuredScopes': 4, 'bindings': 'K10-v2 rev1 / execution rev1',
        'runControl': 'ready', 'evening': out['evening']['report']['status'], 'morning': 'empty', 'providerUnchanged': True,
