"""One-shot, code-only B66 -> B67 cutover. No migration or provider call.
...
RELEASE = 'v3.2.0-b67'
PRIOR = 'v3.2.0-b66'
PRIOR_COMMIT = '6eab734c939f63a7813a4ad9dd8d65ac9b09b6f6'
PAYLOAD = Path('/tmp/neckline-b67-release')
STAGE = PAYLOAD / 'staged'
BACKUP = ROOT / 'data/backups/v3.2.0-b67-predeploy'
BINDING = Path('/etc/neckline/k10.env')
ARCHIVE = 'Neckline-v3.2.0-b67-Backend.tar.gz'
...
def paused(api_active):
    assert h.unit_state('neckline-trial-20260910-evening-b66.service') == 'inactive'
    ...
    with sqlite3.connect(DB.as_uri() + '?mode=ro', uri=True) as conn:
        ...
        assert conn.execute('SELECT status FROM k10_tasks WHERE task_id=?', ('task_c9c0feab5c83a08e8a17138ebfb0041a',)).fetchone() == ('completed',)
        push = json.loads((ROOT/'data/backups/v3.2.0-b66-predeploy/report-push-receipt.json').read_text())
        assert push['notificationState'] == 'sent' and push['deliveredDeviceCount'] > 0
        assert any(x['ok'] and x['status'] == 200 for x in push['apnsResponses'])
