"""B64 read-only release acceptance; offline mode denies all socket connections."""
import argparse
import json
import os
...
if args.mode == 'offline':
    ...
    os.environ['API_TOKEN'] = 'offline-b64-release-token'
...
def verify(get):
    ...
    for window in ('evening', 'morning'):
        ...
        if window == 'evening':
            assert report.report.status == 'failed' and report.reason.reason == 'research_snapshot_missing'
    ...
    assert health['releaseSet'] == 'v3.2.0-b64' and health['status'] == 'ok'
    out['health'] = health
