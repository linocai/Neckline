...
**✅ 已收口(commit `a8a14c3`,2026-08-03,@builder;权威规格改走 planner `88e1e17` 对
⑧-G-D 第②条的修订,不是本条原文的一行 tie-break 建议)**:核实结论比本条原判更重 ——
`consec_limit_up_days` 降序**不只是不确定,是真有偏**(⑦-K7 审计实证连板高度是双尾
放大器,次日跌停约 3× 于同簇其余成员,而这份样本正好喂 `compute_breadth_snapshot`
算跌停数 / 炸板率),故**修法不是给 `consec_limit_up_days` 加 `ts_code` 二级 tie-break**
(那样仍会把有偏的 consec 留作主键、只治好确定性不治偏差),**而是整个截断序换成
`crc32(ts_code)` 升序**(复用 `mainline.crc_rank`,与主线切片姊妹样本同一套采样键)。
双载入(重写分区行序反转)逐位确定性单测已补。**改前改后同批历史日对拍显示 zaban_rate
读数有实质变化**(07-21→07-22 真实配额点 42.9%→33.3%,−9.52pp)——追加全月探索性
对拍(10 个真正截断的日子)证实这是**抽样噪声**(均值 −0.99pp、方向不系统)而非 crc32
新引入的系统性偏差,但暴露了"71 只保底配额下 zaban_rate 本身抽样噪声不小"这一更根本
的事实(旧规则同样承受这份噪声,只是此前不可复现故未被量出来过)。完整数字见
`PROJECT_PLAN.md` §五 ⑧-G 块尾追加记录,已并入 §七 [P3-37]。
...
**✅ 顺手量化(commit `a8a14c3`,2026-08-03,@builder)**:🟡-N1 收口时的历史日对拍已
覆盖这一问——⑧-G 完工记录发布的两个真实配额点(07-21→07-22 配额 71、07-23→07-24
配额 125)上,`limit_down` 计数改前改后均**无变化**;全月探索性对拍(K=71 固定口径,
10 个真正截断的日子)显示 limit_down 变化(新−旧)为 `[1,2,2,0,0,1,2,0,0,0]`(全部
≥0,方向性观察但 n=10 样本小、不作定论)。数字见 `PROJECT_PLAN.md` §五 ⑧-G 块尾。
...
> **✅ 2026-08-03 均已处理(commit `a8a14c3`,@builder)**:🟡-N1 核实后修法比"一行"更深
> (整个截断序换 `crc32`,详见上文);🔵-N2 顺手量化(两个真实配额点 limit_down 无变化,
> 全月探索性对拍附加数字)。两条均已并入 `PROJECT_PLAN.md` §五 ⑧-G 块尾追加记录 + §七
> [P3-37]。
