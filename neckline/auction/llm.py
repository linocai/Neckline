...
🔴 **三道机械夹逼闸**(`clamp_verdict`,⛔ 不靠 LLM 自觉):
    闸 1 **关键域**数据缺失只能形成中性(**无例外**)· 闸 2 Z1 只有一只竞价强股时保持
    中性 · 闸 3 Y1 的否决须「命中失效位 **或** 三项一致负面」。
三道闸的输入**都是 K8 §二十 原文明令的形式约束,没有一个新数字**(§五 ⑨-A 第 6/7 行)。
🔴 **V2.4.0 P2.3 只改了闸 1 的判据源**(整体 → 关键域,`mech.critical_quality`);
闸 2 / 闸 3 **一字未动**,闸 2 的线码判定仍走 `engine_line_of()`(⛔ 别改回枚举版本号)。
...
只有一只高位股强、板块协同不足;**关键域数据质量不是 `ok`**;Y1 平台结构仍完整但竞价表现偏弱。
**关键域数据缺失或冲突只能形成中性;上下文域缺失只降低置信度,不改变结论。**
...
**数据质量分域**(K8 §二十,这一段决定你什么时候必须转中性、什么时候只需要披露):
- **关键域** = 篮子成员自身的竞价数据、每只成员**实际使用的**市场基准、**实际用于**相对板块计算的板块基准、
D0 失效判断所需的冻结锚。**关键域不是 `ok` 时,确认和否决统一转为中性。**
- **上下文域** = 其他市场指数、**未实际用于**当前成员计算的对照股、市场锚点、历史比较、近期量能背景。
**上下文域降级⛔ 不改变结论**,只降低置信度,并且**必须在理由或风险里把缺了什么说出来**。
- 资料里标「读数不合格」的标的:它**抓到了数,但那份数没通过校验**(可能是上一交易日的缓存)——
⛔ 一律不得当作今天的竞价结果引用,也⛔ 不得因为"有个数"就当它没问题。
- 资料里标「跨源冲突」的标的:两个行情源对同一件事给出了**相反的结论**——⛔ 不得高置信输出。
- 🔴 **无关字段缺失⛔ 不得掩盖已有的失效事实**:标了「已触发 D0 明确失效位」的票,不论数据质量如何都要照实讲。
...
def clamp_verdict(fields: Optional[BasketFields], mech: Any) -> Tuple[str, Optional[str]]:
    ...
    **全为 `True`**(`null` = 判不出,**不算负面**);闸 1 没有任何数字。
    ...
    # 🔴 **V2.4.0 P2.3:判据由「整体」收窄到「关键域」**(施工图 §五 P2.3)——
    # 「一只无关指数缺失导致整篮强制中性」正是本版要修的第 ② 个病。上下文域降级
    # **只降置信度 + 披露缺失**,⛔ 不再夹逼结论。
    # ⚠ 之所以仍可以无例外:「命中 D0 失效位」走 §五 ②-G 的**独立警报通道**
    # (`hit_invalidation_json` 机械段就落库、恒定进第 4 块与推送),被夹成 neutral
    # 时那条信息**一个字都没丢**。
    # 🔴 **读 `critical_quality`,⛔ 不回退到 `data_quality`**:两者在新链路上恒等
    # (`_build_basket_mech` 一处赋值),而**取不到这一位就当作不 ok**(默认拒)——
    # 一个没有关键域读数的对象,凭什么让它拿到 confirm / veto?
    if getattr(mech, "critical_quality", None) != DQ_OK and raw != VERDICT_NEUTRAL:
