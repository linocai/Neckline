...
🔴 **有效样本单位 = `D0 日期 × 篮子 × 引擎版本`**(K8.md §十七;V2.3.2-④-B 钉死)
    本表的 `basket_id UNIQUE`(一篮一行)正是这个定义的**物理承载** —— 下游
    `eval/iteration.py` 的 `n` 数的就是这张表的行数,`stratum_of()` 再按引擎码 ×
    引擎版本分层。⛔ **别把这张表改成一票一行 / 一次判定一行**:那会让"样本量"
    在不改任何统计代码的情况下悄悄变成另一个量(成员多的篮子权重变大),
    而 `n≥30 / n≥80` 那两条分界线是按**篮子数**拍板的。
