"""⛔ **DEPRECATED(V2.4.0 P0 退役)—— 生产链零调用。**

    本模块是**退潮红色刹车**「主线板块跳水」那一路的样本派生,退潮判级整体退役后
    它**没有任何生产消费方**(见 `sentinel/retreat.py` 模块头)。`sentinel/universe.py`
    对它的 import 与两处调用(`derive_mainline_sample` / `crc_rank`)已一并摘除。

    · **为什么文件还在**:既有单测把它当行为基准读 + 回滚绳(§3.14-A)。
    · ⚠ **`crc_rank` 仍是全项目 crc32 采样排序的唯一实现** —— 下一次要做「无偏截断」
      的取样时**回来用它,⛔ 别另抄一份**;下面那段「样本即判据、不得沾 LLM」的自查
      判据(「换一批成员,这个数会不会变?」)是**通用纪律,没有随本模块退役而失效**。
...
之一,红色 = **一条全局刹车 + 立即级推送 = 纪律层动作**(⛔ 该动作语义已随 V2.4.0 P0
撤销)。V1 该样本 =「关注池里命中
