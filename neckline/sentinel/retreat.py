"""⛔ **DEPRECATED(V2.4.0 P0 退役)—— 生产链零调用,本模块不再产生任何新判级。**

    · **退役的是什么**:代理关注池 → 「大盘退潮」→ 一条**作废当日计划 + 停止开新仓**
      的全局刹车。审计规格 P0.1 逐条列的结构问题:统计样本是**持仓 / T1/T2 /
      主线切片 / 昨日涨停**这些策略关注对象、**不是全市场**;今日与昨日的关注池成员
      会变,「昨日同时段」不是同一批股票的可比样本;约 60s 一拍的「连续两拍」只代表
      约两分钟;红事件一旦落库**全天闩锁**,不再因后续修复重判;没有 brake 就显示
      「运行正常」,分不清真正常 / 行情过期 / 服务停摆 / 本拍无数据。
      **一句话:测量范围与动作权限不匹配。**
    · **退役 = 撤销判断权**,⛔ 不是调阈值 / 改文案 / 改颜色 / 加状态数。
    · **断链落点**:`sentinel/engine.py::run_tick` 原第 1 段整段删除;
      `sentinel/universe.py` 的两份专用测量样本(主线切片 / 昨日涨停宽度)与池位配额
      一并删除;`api/notify.py::push_retreat_brake` 从生产链摘除且
      `KIND_RETREAT ∈ notify_kinds.RETIRED_KINDS`(扇出层直接拒发);
      `retreat_metrics` 表**停写、历史行只读保留、⛔ 不 DROP**。
    · ⚠ **心跳证据换人**:此前「盘中哨兵还活着」看的是 `retreat_metrics` 每拍 +1,
      停写后改看 `sentinel_events` 的 `sentinel='capture'` 行 + journal 的 tick 日志。
    · **为什么文件还在**:既有单测把它当行为基准读,且是回滚绳的一部分(§3.14-A)。
      **判据是「生产入口有没有它」**:守门单测 AST 扫 `engine.py`,
      `evaluate_retreat` / `compute_breadth_snapshot` / `record_retreat_metrics`
      调用点数必须为 0。
    · ⛔ **不许换名复活**:⛔ 不许「另一种名称的大盘退潮」、⛔ 不许「另一种名称的
      建议暂停开仓」、⛔ 不许盘中风险分数 / 正常-观察-恶化-恢复状态机 / 红黄绿交通灯。

🔴 **「retreat」在本仓有三个含义,本模块是 ①**:① 本模块 + `KIND_RETREAT` = 代理池
退潮刹车(**已退役**)· ② `retreat_metrics` 表 = 历史审计行(**只读保留**)·
③ `retraceState` / `take_profit_retrace` = **回落止盈**(拼写相近、语义无关,⛔ 别碰)。

以下为退役前的原文说明(**历史留痕,行为已从生产链断开**):

退潮哨兵(plan §2.4 第2条)。「盘中情绪恶化(炸板率飙升/跌停家数急扩/主线板块
跳水)→ 推一条红色刹车(原文案已随 V2.4.0 P0 按 P0.7 判据 #2 从全仓清除)」。
触发后应联动买点哨兵(engine.py
