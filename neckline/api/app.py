...
# 🛑 **本版号尚未上云,别照着 VERSION 推断生产在跑什么**(2026-07-29):第 ⑨ 块完整部署
# 尝试过并**已回滚** —— 生产现在跑的是 `v1.4.0-p1`(git `fc55e2e` 代码态)。阻塞 =
# §七 **P0-23**(`compute_industry_strength` 扫全历史 784 万行,生产 2 vCPU/1.6G 上
# 700M cap OOM-kill、1400M cap 600s 跑不完;16:35 报告主链与 info-card/问询台三处全中招)。
# ⚠ **P0-23 未修之前不要再跑 `scripts/sync_code.sh` 把本代码推上去**,推上去当天大概率无报告。
# 详情与回滚证据见 PROJECT_PLAN §四 快照 + §九 对应一行。
#
# v1.4.0(本地代码态):`-p1` 半版状态结束——① 之后的 ②~⑧ 全部就位
