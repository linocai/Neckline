...
class LLMProvider(ABC):
    ...
    def chat(
        ...
        **为什么需要它**:2026-07-27 生产实测,GLM 推导出的检索词紧跟**最后一条 user
        消息**;问询台的最后一条恰好是用户的代词提问(「这只票最近业绩怎么样?」),
        身份信息在更早的材料消息里救不回来 → 搜出来的是泛泛的板块新闻,模型只好
        退回训练数据答。同一条链路只要把股票名+代码放进检索词,命中立刻全变成该股
        的真实新闻。详见 `api/inquiry.py` 调用点注释。"""
