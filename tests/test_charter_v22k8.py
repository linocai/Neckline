...
class TestMaxHoldDaysWidening:
    ...
    def test_k1_exit_behaviour_bit_identical_after_widening(self):
        """K1 逐位不变护栏(逻辑层)—— 与 `tests/test_v13_exit_guardrail.py` 同一组判据,
        这里再钉一次「放宽类型没改默认档行为」。

        ⚠ **六年真回测那一层已无载体**:`tests/test_v13_exit_6y_baseline.py` 依赖已随策略
        档案迁出的 `research/lab.py` + `k3_panel.parquet`,在本仓**永久无法复跑**(恒 skip,
        §1.6 / §七 **P4-54**)。故本仓能跑的护栏只有逻辑层这一道,完工报告已如实说明。"""
        from tests.test_v13_exit_guardrail import _date_at_held, _empty_strategy, _pos
        from neckline.strategy.momentum import MomentumConfig
        ...
        r = s._exit_reason("X", _pos(), _date_at_held(5), {"X": 12.0}, {"X": 12.0})
        assert r is not None and "时间退出" in r
        assert s._eff_max == {}          # 默认档绝不触碰豁免态
...
class TestNoTimeExitNeverFires:
    def test_backtest_side_never_time_exits(self):
        """回测侧:D5 / D15 / D50 一律不因持有天数卖出,`_eff_max` 永不被写。"""
        from tests.test_v13_exit_guardrail import _date_at_held, _empty_strategy, _pos

        s = _empty_strategy(_k8_cfg())
        for n in (1, 5, 15, 50):
            assert s._exit_reason("X", _pos(), _date_at_held(n), {"X": 10.0}, {"X": 10.0}) is None
        assert s._eff_max == {}

    def test_backtest_side_stop_loss_still_wins(self):
        """⚠ **止损没退役**:`stop_pct=0.05` 照常触发(值与唯一源地位一字不动)。"""
        from tests.test_v13_exit_guardrail import _date_at_held, _empty_strategy, _pos

        s = _empty_strategy(_k8_cfg())
        r = s._exit_reason("X", _pos(), _date_at_held(3), {"X": 9.0}, {"X": 9.0})
    ...
    def test_backtest_side_take_profit_never_fires_when_none(self):
        from tests.test_v13_exit_guardrail import _date_at_held, _empty_strategy, _pos

        s = _empty_strategy(_k8_cfg())
        s._peak_close["X"] = 20.0        # 从 20 回落到 10.5 = −47.5%,若有 8% 阈早该触发
        assert s._exit_reason("X", _pos(), _date_at_held(3), {"X": 10.5}, {"X": 10.5}) is None
