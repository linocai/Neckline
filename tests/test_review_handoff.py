...
class TestRenderHandoff:
    ...
    def test_disclaimer_is_reused_not_rewritten(self):
        from neckline.eval.calibration import DISCLAIMER
...
def test_artifact_naming_matches_the_writer_byte_for_byte():
    """产物命名由 `eval/calibration.py::write_report` 定死,⛔ 本模块不另起一套。
    体例:直接从 writer 源码里读出那行 f-string 的形状,与本模块的前缀对拍。"""
    writer = (_ROOT / "neckline" / "eval" / "calibration.py").read_text(encoding="utf-8")
    m = re.search(r'stem or f"([a-z_]+)\{report\.date_from\}_\{report\.date_to\}"', writer)
    assert m, "`write_report` 的产物命名行变了,来这里同步 `_ARTIFACT_PREFIX`"
    assert m.group(1) == ho._ARTIFACT_PREFIX
