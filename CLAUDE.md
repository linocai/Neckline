...
## 🔴 三处「同名不同物」——碰盘中退役件之前先认清是哪一个(V2.4.0 P0 **已执行**)

- **`invalidation` 有三个**:① `sentinel/invalidation.py` = **盘中通用证伪**(VWAP / 低开未翻红 /
  折算量比)· ② 卡上 `invalidation_spec` / `close_below_stop_line` = **D0 冻结的判断失效位置**
  (K8 §十一 交易资格四件套之一)· ③ `auction/mech.py::hit_invalidation` + `decision_log.invalidation`。
  **V2.4.0 P0 只退役 ①**;②③ 是明令保留的能力。🔴 **裸 `grep invalidation` 会把三者一起捞上来 ——
  按文件与符号删,⛔ 不按词删。**
- **`retreat` 有三个**:① `sentinel/{retreat,retreat_store}.py` + `KIND_RETREAT` = 代理池退潮刹车
  (P0 停写断链)· ② `retreat_metrics` 表 = 历史审计行(**只读保留,⛔ 不 DROP**)· ③ `retraceState` /
  `take_profit_retrace` = **回落止盈**(拼写相近、语义无关)。
- **`board` 有三个**:① `GET /api/v1/board` + `BoardSection` = 盘中动态页(P0 删)· ②
  `data/board.py::classify` / `CandidateOut.board` = **上市板块** `MAIN/GEM/STAR/BSE`(⛔ 一行不动)·
  ③ `boardAge` = 概念板块年龄(⛔ 不动)。
- ⚠ **`retreat_metrics` 停写的副作用**:它此前是「盘中哨兵还活着」的心跳证据(部署核查用过)——
  停写后改看 `sentinel_events` 的 `capture` 行 + journal tick 日志。→ §七 **P1-75**。

## V2.4.0 P0 退役实施后的四个新坑(2026-08-12 施工实打)

- 🔴 **「留文件断链路」的退役件,守门必须分两种口径扫,⛔ 不能一把梭**:①「**文案**」类判据
  (P0.7 #2)**连注释与 docstring 一起扫、零豁免名单** —— 所以退役说明里⛔ 不许原样引用被删的那几句话
  (施工中为此改写了 9 处历史叙述);②「**符号零引用**」类判据(P0.7 #4/#5)必须**先剥行注释**
  —— 一条写着「这个组件已删除」的注释是留给下一个人的说明,把它算成引用 = 逼注释绕开自己要说的名字
  = `CLAUDE.md`「对自己的注释报警的闸门等于没有闸门」的翻版。两种口径**刻意相反**,写守门前先分清。
- 🔴 **退役观测位要物理删,⛔ 不许留一个恒 `False` 的字段**:`TickResult` 的 5 个退潮/证伪位
  (`retreat_active`/`retreat_alert`/`retreat_warning`/`breadth_snapshot`/`invalidation_signals`)整体删除,
  守门按 `__dataclass_fields__` 反向断言「一个都不许长回来」。留一个恒 False 的位就是
  「前端隐藏、后台仍在判」的温床(而且**编译不报错、单测也测不出**)。
  同理**推送退役走「加一张表」**:`RETIRED_KINDS` 与冻结元组 `ALL_KINDS` 并列,闸门放在**唯一扇出路径**
  `push_event` 上、且排在**开关闸之前**(开着也发不出去);⛔ 不从 `ALL_KINDS` 里删(旧客户端 PUT 会 422)。
- 🔴 **关注池缩编会连坐竞价层,而且看不出来**:两份退潮专用样本删掉后 `wu.codes` 由约 200 只 → 上界 29 只,
  而 `auction/collect.py::build_watchlist` 把 `wu.codes` 当**竞价强势股**与**板块对照股**两个取样域 ——
  「对照不足」由常态变近乎必然(§七 **P1-78**,**是必然连带不是 bug**)。⚠ **同一刀还静默废掉了
  `scripts/smoke_auction.py` 的合成篮子**(它原本取 `wu0.codes`,缩编后在零篮子零持仓的开发库里恒空)
  → 闸 2 一行走不到而**冒烟看起来照常跑通**;已改成从 D0 `daily` 分区取码。**判据:改完真跑一遍两条冒烟,
  看它有没有真的走到该走的分支,⛔ 别只看退出码 0。**
- **服务端下发的时刻串是 UTC ISO(25 字符)**,直接进 `Text` 又长又不像时刻
...
## V2.4.0 P2 施工实打的五个新坑(竞价数据可靠性;碰 `auction/quality.py` 之前先读)

- 🔴 **`ts` 一旦进了判据,所有合成读数都得给一个能解析的时刻**:P2.1 起七项校验会看 `Quote.ts`,
  而 `smoke_auction.py` 老版本合成的是 `"集合竞价 合成"` → `timestamp_unparseable` → **整份快照
  零可用读数、冒烟空跑而退出码仍是 0**。合成一律写 `<D1> 09:25:03`(落在 K8 的 `[09:25, captured_at]` 内)。
- 🔴 **K8 第 ⑤ 项「open/price/pre_close 必要字段」⛔ 不能一档到底**:现价 / 前收盘是竞价涨跌幅的
  分子分母(缺了整条读数都算不出)= **致命**;开盘价只被失效位判定用,而那一项**本来就有第三态**
  (`UNDET_NO_OPEN_PRICE`)= **非致命**。一档到底会因为一个用不上的字段把好的价 / 量 / 额一起扔掉
  —— 用一个新病换掉一个老病。`freshness` 因此是**四态**(多一个 `degraded`),⛔ 别"简化"回三态。
- 🔴 **不合格的读数「留痕但不派生」**:`snap.quotes` 只装通过校验的(→ `gap_of` / 逐票读数一律
  `None`),两源原始数字**全在** `quote_quality_json` 里。判「这一格是没抓到还是抓到了不能用」
  **取 `quote_quality[code].checks` 有没有内容,⛔ 不取 `q is None`** —— 不合格的读数也不在 `quotes` 里。
- 🔴 **「关键域」的三条判据全是「**实际用到的**」**:成员自己的市场基准(科创板按 K8 §三 排除 →
  它**没有**基准,⛔ 不算缺失)· **算出来了**的板块对照股(**对照不足 = 压根没用上 → ⛔ 不进关键域**,
  P1-78 那条连带因此不会把整篮拖成中性)· D0 冻结锚(**有篮无卡 / 卡上没冻结失效位 = 新增的夹逼
  触发面**,K8 §二十 逐字,方向刻意保守)。
...
- 🔴 **跨源冲突自 V2.4.0 P2.2 起真的会有值**:`get_quotes_dual()` 双源批量核验已上线
  (sina 一次 + tencent 一次,**净增 1 次 HTTP 请求 / 早晨**)。⛔ **V2.3.3「结构性恒空、
  不加第二次请求」的旧口径已被 K8 §二十「有界双源核验」推翻**(§七 P4-66 改判)——
  代码注释、DDL 注释、客户端文案里那句话都改过了,⛔ 别照旧注释办事。
  ⚠ `get_quotes()` **一字未动**(普通上下文股票仍是「主源失败才降备源」,守门正面锁)。
...
- **V2 文档层级(防第三份权威;2026-08-05 V2 收官后的常态口径)**:**现行口径权威只有
  ...
  `archive/新版本量化交易APP与选股架构.md`(⚠ **2026-08-12 已由根目录移进 `archive/`**,连同
  `archive/Neckline视觉升级/` 原型目录;旧文里的根目录路径一律按此读)= 产品语义蓝图
  (可读、不可当施工口径),`archive/V2架构设计稿_*.md`
