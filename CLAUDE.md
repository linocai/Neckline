...
- **三条版本线(2026-08-09 K8 入仓起,冷启动必读,写号前先分清是哪条)**:① **系统线 `v` 字头**
  ...
  §2.1,切换器 `activate_charter.py`)③ **K8 选股线**(权威需求 `K8_STRATEGY_ARCH.md`,实现口径
  `PROJECT_PLAN.md` §五 V2.2.0)—— 它自己又分**骨架 `K8-V0.5`** 与**引擎 `C1`/`Z1`/`Y1` 各自独立**。
  ⛔ **`V0.5` 禁简写**(与满篇 `v` 字头只差一个大小写);⛔ 引擎升级写 `C2`/`Z2`/`Y2`,**不写「K8 v2」**。
  ⚠ `K8_STRATEGY_ARCH.md` 是**原件快照、不追改后续裁定**,已知三处要看施工图才完整:六关的机械 /
  证据二分 · 行情状态第五维的反馈环断开 · 章程退役机械止盈与时间退出的风险登记。
  ⚠ **K8 由用户直接转交系统线,没走策略线立项流程** → `STRATEGY_LAB.md` §四 仍停在 K7,**别拿它当
  K8 的背书**(§七 P3-53);K8 的假设**零回测背书**,位置关还正撞在 K3 已判死的域上(§七 **P3-49**,
  施工前必读)。
